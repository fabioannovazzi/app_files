...
| Audit Reconciliation | Rights-cleared public expenditure slice split into independently sealed open-item and payment views | A second public transaction population selected after implementation freeze | Reviewed source roles, one-to-one evidence use, exact residuals, party/currency controls, final-readiness blocking | Ambiguous roles, duplicate or reused evidence, fan-out, party mismatch, cross-currency allocation, truncation |
