...
try:
    ...
    logging.getLogger(__name__).warning(
        "common_data_utils helpers import error: %s", e
    )
...
def transform_lazy_df(
    ...
    if plotValuesAsChoice in chartDict and chartDict[plotValuesAsChoice] == absolute:
        lazy_cols, _ = get_schema_and_column_names(lazy_df)
        cols_to_unpivot = [col for col in lazy_cols if col != dimensionName]
...
def add_row_to_dataframe(
    ...
    if lengthDiff > 1:
        rowArray = rowArray + [np.nan] * lengthDiff
...
def pivot_lazy(
    ...
    def _pivot_df(df: pl.DataFrame, agg_key: str) -> pl.DataFrame:
        ...
        attempts = [
            ((), {"columns": pivot_col, "index": index_col, "values": value_col, **agg_kwargs}),
            ((), {"on": pivot_col, "index": index_col, "values": value_col, **agg_kwargs}),
    ...
    for idx, column in enumerate(columns):
        ...
        if idx != 0:
            for metric in metricsToPlot:
                new_col = new_col.replace(metric, "")
...
def rank_others_as_last(
    ...
    lf = lf.with_row_index(rank).with_columns(
        pl.when(is_other_label)
        .then(rank_value)
        .otherwise(pl.col(rank) + 1)
        .alias(rank)
    )
...
def get_month_name(df):
    ...
    df = df.with_columns(
        pl.col(dateName).dt.month().map_dict(monthDict).alias(dateName)
    )
    return df
...
def show_only_largest(
    ...
    if numberOfUniqueDimension > numberOfTop:
        ...
        if aggregateOtherItems:
            ...
            df = df.filter(pl.col(rank) == metConditionValue)
            df = drop_columns(df, [rank])
    else:
        df, group_byCols, valueCols, uniqueItems = (
