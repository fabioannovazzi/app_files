...
def make_stacked_column_chart_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    ...
    companyName,paramDict,chartDict=get_filter_text_or_company_name(chartDict,paramDict)
    ...
    if element:
        ...
        period1=get_rolling_and_year_to_date_period(period1,paramDict,chartDict,False) 
        period=period1
    ...
    if overlayChartMetricKey in chartDict and metricsToPlot in chartDict and len(chartDict[metricsToPlot])>1:
        secondMetric=change_metric_if_cost_analysis(chartDict[metricsToPlot][1],chartDict)
        linecurrencyName,paramDict=get_currency_name(chartDict,paramDict,chartDict[metricsToPlot][1]) 
        chartDict=explain_metrics_for_stacked_column_prompt(chartDict,currencyName,linecurrencyName,metric,secondMetric)  
        if linecurrencyName:       
            linecurrencyName=" in "+linecurrencyName
        overlayOne= "Bar chart: "    
        overlayTwo=". Line chart: "+"<b>"+secondMetric+"</b>"+linecurrencyName        
    title=companyName+breakTag+overlayOne+"<b>"+metric+"</b>"+currencyTag+overlayTwo+dimension+" "+likeForLikeMessage+breakTag+period     
    title=title.replace("  "," ")
    chartDict[plotTitleText]=title
    return title,paramDict,chartDict

def make_stacked_pareto_and_pareto_chart_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"] 
    breakTag=set_break_row_tag(chartDict,chosenChart)
    companyName,paramDict,chartDict=get_filter_text_or_company_name(chartDict,paramDict)
    ...
    if period1 == periodOrder[0]:
        isPeriodZero=True
    period1=get_rolling_and_year_to_date_period(period1,paramDict,chartDict,isPeriodZero)
    if aggregateUniquesByDimension in chartDict and chartDict[aggregateUniquesByDimension]: 
        rankingMetric=", count by "+chartDict[countColumn]
    metric=change_metric_if_cost_analysis(metric,chartDict)  
    currencyTag=""      
    if currencyName:       
        currencyTag=" in "+currencyName+""
    title=companyName+breakTag+"ABC by sorted "+dimension+" "+metric+" "+currencyTag+rankingMetric+likeForLikeMessage+breakTag+period1 
    title=title.replace("  "," ")
    chartDict[plotTitleText]=title    
    return title,paramDict,chartDict

def make_distribution_charts_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"] 
    ...
    metric=change_metric_if_cost_analysis(metric,chartDict)
    likeForLikeMessage=make_like_for_like_title_suffix(chartDict,paramDict,metric)
    if distributionDimension in chartDict and chartDict[distributionDimension] != nothingFilteredName:
    ...
    if currencyName:       
        currencyTag=" in "+currencyName+", "    
    title=companyName+breakTag+"<b>"+metric+"</b> "+currencyTag+distributionDimension+dimension+" "+likeForLikeMessage+breakTag+period1+" vs "+element   
    title=title.replace("  "," ")  
    chartDict[plotTitleText]=title
    return title,paramDict,chartDict
...
def make_slope_and_dot_chart_title(
    ...
    if metricsToPlot in chartDict and (not dimension or dimension == totalName):
        ...
        metric=change_metric_if_cost_analysis(metric,chartDict)     
    if datePeriodName in paramDict and paramDict[datePeriodName] in [quarterName,monthName,weekName]:
        try:
    ...
    if currencyName:       
        currencyTag=" in "+currencyName+" "                                
    if dimension == totalName:
        title=companyName+breakTag+"<b>"+metric+"</b>"+currencyTag+likeForLikeMessage+breakTag+period1+" vs "+element    
    else:    
        title=companyName+breakTag+"<b>"+metric+"</b>"+currencyTag+likeForLikeMessage+" by "+dimension+" "+breakTag+period1+" vs "+element 
    title=title.replace("  "," ") 
    chartDict[plotTitleText]=title   
    return title,paramDict,chartDict

def make_marimekko_and_stacked_bar_chart_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    ...
    if compareScenariosOrPeriods in chartDict and chartDict[compareScenariosOrPeriods]==compareScenarios:
        duration=str(chartDict[dateRangeArray][0]).replace(hyphenName,"/")+"-"+str(chartDict[dateRangeArray][1]).replace(hyphenName,"/")
    ...
    if compareScenariosOrPeriods in chartDict and chartDict[compareScenariosOrPeriods]==compareScenarios: 
        duration=get_rolling_and_year_to_date_period(duration,paramDict,chartDict,False)
    elif duration == period1:    
        duration=get_rolling_and_year_to_date_period(duration,paramDict,chartDict,False)
    else:
        duration=get_rolling_and_year_to_date_period(period1,paramDict,chartDict,True)      
    separator=" and "
    if yAxisDimension in chartDict and chartDict[yAxisDimension] in [nothingFilteredName]:
        separator,element="",""
    if chosenChart in [barmekkoChart]:
        ...
        areaMetric=". Bar area: "
        multipliedMetric=change_metric_if_cost_analysis(chartDict[multipliedMetric],chartDict)
        thirdCurrency,paramDict=get_currency_name(chartDict,paramDict,multipliedMetric) 
        thirdCurrencyTag=""
        if thirdCurrency:       
    ...
    if chosenChart in [barmekkoChart]:
        ...
        chartDict=explain_metrics_for_barmekko_prompt(chartDict,currencyName,secondCurrency,thirdCurrency,metric,element)
        title=companyName+breakTag+"Bar length: "+"<b>"+metric+"</b>"+currencyTag+likeForLikeMessage+" by "+dimension+separator+"<b>"+element+"</b>"+secondCurrencyTag+areaMetric+"<b>"+multipliedMetric+"</b>"+thirdCurrencyTag+facet_suffix+breakTag+period
    else:
        if chosenChart in [stackedBarChart,marimekkoChart,barmekkoChart] and plotSmallMultiplesKey in chartDict and chartDict[plotSmallMultiplesKey]:
            if yAxisDimension in chartDict and chartDict[yAxisDimension] in [nothingFilteredName]:
                dimension=chartDict[smallMultiplesColumn]+" and "+dimension
            else:    
                dimension=chartDict[smallMultiplesColumn]+", "+dimension
        title=companyName+"<b>"+breakTag+metric+"</b>"+currencyTag+likeForLikeMessage+" by "+dimension+separator+element+facet_suffix+" "+breakTag+period   
    if overlayChartMetricKey in chartDict and metricsToPlot in chartDict and len(chartDict[metricsToPlot])>1:
        secondMetric=change_metric_if_cost_analysis(chartDict[metricsToPlot][1],chartDict)
        linecurrencyName,paramDict=get_currency_name(chartDict,paramDict,chartDict[metricsToPlot][1])
        chartDict=explain_metrics_for_stacked_column_prompt(chartDict,currencyName,linecurrencyName,metric,secondMetric) 
        ...
        title=companyName+breakTag+overlayOne+"<b>"+metric+"</b>"+currencyTag+overlayTwo+". By "+dimension+"  "+likeForLikeMessage+breakTag+period   
    title=title.replace("  "," ")  
    chartDict[plotTitleText]=title
    return title,paramDict,chartDict

def make_venn_and_upset_charts_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"] 
    selectedPeriods=namingParams["selectedPeriods"]
    periodOrder=chartDict[selectedPeriods]
    ...
    if compareScenariosOrPeriods in chartDict and chartDict[compareScenariosOrPeriods]==compareScenarios: 
        ...
        duration=get_rolling_and_year_to_date_period(period1,paramDict,chartDict,True) 
    period1=get_rolling_and_year_to_date_period(period1,paramDict,chartDict,isPeriodZero)
    title=companyName+breakTag+element+overlap+dimension+likeForLikeMessage+breakTag+period1     
    title=title.replace("  "," ")  
    title=title.replace("<br>"," ")  
    chartDict[plotTitleText]=title
    return title,paramDict,chartDict     

def make_bubble_or_motion_chart_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"] 
    ...
    if chosenChart in [bubbleChart]: 
        ...
        if element == totalName:
            dimension=" by "+dimension
            title=companyName+breakTag+"<b>"+yDimension+", "+xDimension+", "+metric+"</b>"+currencyTag+dimension+" "+likeForLikeMessage+breakTag+period1 
        else:
            dimension=" by "+dimension+" and "+chartDict[smallMultiplesColumn]
            title=companyName+breakTag+"<b>"+yDimension+", "+xDimension+", "+metric+"</b>"+currencyTag+dimension+" "+likeForLikeMessage+breakTag+period1             
    elif chosenChart in [motionChart]: 
        title=companyName+breakTag+"<b>"+yDimension+", "+xDimension+", "+metric+"</b>"+currencyTag+" by "+dimension+" "+likeForLikeMessage+breakTag+period1[0]+" to "+period1[-1]      
        ...
        if isolineMetric in chartDict and chartDict[isolineMetric]:
            ...
            isolineMetricValue=change_metric_if_cost_analysis(isolineMetricValue,chartDict)
            currencyName,paramDict=get_currency_name(chartDict,paramDict,isolineMetricValue)
        ...
        if len(period1) == 1:
            ...
            period0=get_rolling_and_year_to_date_period(period1[0],paramDict,chartDict,True)
            period1=get_rolling_and_year_to_date_period(period1[1],paramDict,chartDict,False)
            period1=str(period0)+", "+str(period1)            
        if dimension == totalName:
            title=companyName+breakTag+"<b>"+metric+", "+element+isolineMetricValue+" </b>"+currencyTag+" "+dotDimension+" "+likeForLikeMessage+breakTag+str(period1) 
        else:
            dimension=dotDimension+" and "+dimension
            title=companyName+breakTag+"<b>"+metric+", "+element+isolineMetricValue+" </b>"+currencyTag+" "+dimension+" "+likeForLikeMessage+breakTag+str(period1) 
    title=title.replace("  "," ")
    chartDict[plotTitleText]=title
    return title,paramDict,chartDict

def make_vertical_waterfall_chart_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"]
    selectedPeriods=namingParams["selectedPeriods"]     
    ...
    if varianceDifferentCalculations in chartDict and chartDict[varianceDifferentCalculations]:
        if varianceAggregation in  [priceAndUnitsAggregation,priceAndVolumeAggregation,totalVarianceAggregation,totalVariance,mixAndUnitsAggregation,mixAndVolumeAggregation ] or varianceAggregation in  [priceAndUnitsAggregation.title(),priceAndVolumeAggregation.title(),totalVarianceAggregation.title(),totalVariance.title(),mixAndUnitsAggregation.title(),mixAndVolumeAggregation.title() ]:
    ...
    title=companyName+breakTag+"<b>"+varianceAggregation+" </b>"+currencyTag+dimension+likeForLikeMessage+breakTag+period 
    title=title.replace("  "," ")
    chartDict[plotTitleText]=title    
    return title,paramDict,chartDict

def make_horizontal_waterfall_chart_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    selectedPeriods=namingParams["selectedPeriods"]
    totalName=namingParams["totalName"]
    fcName=namingParams["fcName"]
    ...
    if currencyName:       
        currencyTag=" in "+currencyName+" "     
    title=companyName+"<b>"+breakTag+metric+"</b>"+currencyTag+dimension+" "+likeForLikeMessage+breakTag+period 
    title=title.replace("  "," ")
    chartDict[plotTitleText]=title    
    return title,paramDict,chartDict

def make_timeline_and_area_charts_title(dfCopy,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"] 
    ...
    if dimension == totalName or len(dimension)==0:
        title=companyName+breakTag+"<b>"+metric+" </b>"+currencyTag+likeForLikeMessage+breakTag+firstPeriod+" to "+lastPeriod    
    else:    
        title=companyName+breakTag+"<b>"+metric+" by "+dimension+" </b>"+currencyTag+likeForLikeMessage+breakTag+firstPeriod+" to "+lastPeriod   
    title=title.replace("  "," ")
    chartDict[plotTitleText]=title    
    return title,paramDict,chartDict 

def make_alternative_combinations_charts_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"] 
    ...
    metric=change_metric_if_cost_analysis(metric,chartDict)
    likeForLikeMessage=make_like_for_like_title_suffix(chartDict,paramDict,metric)
    title=companyName+breakTag+"<b>"+"Alternative combinations"+" </b>"+likeForLikeMessage+breakTag+chartDict[rowToPlot]   
    title=title.replace("  "," ")    
    return title,paramDict,chartDict 

def make_multitier_bar_chart_title(df,chosenChart,paramDict,dimension,metric,chartDict,period1,element):
    namingParams=get_naming_params()
    totalName=namingParams["totalName"] 
    fcName=namingParams["fcName"]
    acName=namingParams["acName"]
    ...
    elif periodChoice == quarterName and (compareWithYearBefore not in chartDict or not chartDict[compareWithYearBefore]):
        period1=pqName  
    elif periodChoice == monthName and (compareWithYearBefore not in chartDict or not chartDict[compareWithYearBefore]):
        period1=pmName  
    elif periodChoice == weekName and (compareWithYearBefore not in chartDict or not chartDict[compareWithYearBefore]):
        period1=pwName  
    if compareScenariosOrPeriods in chartDict and chartDict[compareScenariosOrPeriods]==compareScenarios:
            duration=str(chartDict[dateRangeArray][0]).replace(hyphenName,"/")+"-"+str(chartDict[dateRangeArray][1]).replace(hyphenName,"/")
    elif plName in columns or plName in chartDict[selectedPeriods]: 
    ...
    if dimension and dimension != totalName:
        if plotSmallMultiplesKey in chartDict and chartDict[plotSmallMultiplesKey]:
            if xAxisDimension in chartDict:
                ...
                secondDimension=dimensionDisplayLabels.get(secondDimension, secondDimension)
    ...
    if currencyName:       
        currencyTag=" in "+currencyName+" "              
    title=companyName+"<b>"+breakTag+metric+"</b>"+currencyTag+likeForLikeMessage+dimension+breakTag+period
    title=title.replace("  "," ") 
    chartDict[plotTitleText]=title   
    return title,paramDict,chartDict 
