...
from modules.charting.draw_venn_upset import (
    organize_upset_chart,
    organize_venn_chart,
)
        ...
        for column in smallMultiplesColumnArray:
            ...
            if column==totalName:
                ...
                dfSmallMultiples = dfSmallMultiples.with_columns(
                    ...
                    .then(smallMultipleAggregateOtherItemsName)
...
def plot_pareto_chart(dfCopy,chartDict,paramDict):
    ...
    if is_valid_lazyframe(df):
        ...
        for metric in metricsToPlot:
            lf,colorListDict,classColorDict,metric,ratioName=prepare_data_for_pareto(
...
def plot_stacked_pareto_chart(dfCopy,chartDict,paramDict):
    ...
    if is_valid_lazyframe(df) and chartDict[countColumn]:
        ...
        for metric in metricsToPlot:     
            dfDict[metric],colorListDict,classColorDict,metric,ratioName=prepare_data_for_pareto(df,period,metric,chartDict,paramDict,colorListDict,classColorDict,count) 
            count=count+1    
        dfJoined=join_metric_dataframes(dfDict,metricsToPlot)
        ...
        if showMetricsInDataColumn in chartDict and chartDict[showMetricsInDataColumn]:
            df = df.with_columns(pl.col(countByColumn).cumsum().alias(cumSum))
        ...
        if column in columnsToPlot and column != totalName:
            ...
            if numberOfItemsInCol > 1 or column==totalName or not plottedSomething :
                  ...
                  for metric in [chartDict[singleMetric]]:
                        ...
                        df = df.with_columns(
                            pl.col(periodName).map_dict(order_map).alias("_ord")
                        )
                        ...
                        title,paramDict=make_slope_and_dot_chart_title(df,dotChart,paramDict,column,metric,chartDict,checkedPeriodOrder[0],checkedPeriodOrder[1]) 
