...
def process_node_combinations(dfCopy, indexCols, paramDict, chartDict, run):
    ...
    if dfArray.__len__() > 0:
        df_list_concat = (
            pl.concat([ensure_lazyframe(f) for f in dfArray])
            if use_lazy
            else pl.concat(dfArray)
        )
        ...
        df_list_concat = pl.LazyFrame() if use_lazy else pl.DataFrame()

    if len(detailsDfArray) >= 1:
        df_details_concat = (
            pl.concat([ensure_lazyframe(f) for f in detailsDfArray])
            if use_lazy
            else pl.concat(detailsDfArray)
        )
    else:
        df_details_concat = pl.LazyFrame() if use_lazy else pl.DataFrame()

    if len(inbondsDfArray) >= 1:
        df_snapshot_concat = (
            pl.concat([ensure_lazyframe(f) for f in inbondsDfArray])
            if use_lazy
            else pl.concat(inbondsDfArray)
        )
    else:
        df_snapshot_concat = pl.LazyFrame() if use_lazy else pl.DataFrame()

    return (
        (
            ensure_lazyframe(df_list_concat)
            if use_lazy
            else ensure_polars_df(df_list_concat)
        ),
        (
            ensure_lazyframe(df_details_concat)
            if use_lazy
            else ensure_polars_df(df_details_concat)
        ),
        (
            ensure_lazyframe(df_snapshot_concat)
            if use_lazy
            else ensure_polars_df(df_snapshot_concat)
        ),
        paramDict,
    )
...
def make_filtering_dataframe(
    ...
    use_lazy = isinstance(dfCopy, pl.LazyFrame)
    df_pl = ensure_lazyframe(dfCopy) if use_lazy else ensure_polars_df(dfCopy)

    df_pl = df_pl.select(indexCols + [loopRandomKey, varianceTypeName])

    first_vals_lf = df_pl.select(pl.col(col).first().alias(col) for col in indexCols)
    first_vals = first_vals_lf.limit(1).collect() if use_lazy else first_vals_lf
    first_values = {
...
def get_to_subtract_from_rows(
    ...
    df_pl = df_pl.select(indexCols)

    first_row = df_pl.head(1)
    first_row = first_row.collect() if use_lazy else first_row
    first_values = (
    ...
    if use_lazy:
        return df_pl, paramDict
    return ensure_polars_df(df_pl), paramDict
...
def find_missing_to_subtract_rows(
    ...
    df = duplicate_dataframe(dfCopy)
    df_to_subtract = duplicate_dataframe(dfToSubtractCopy)
    ...
    df = (
        df.join(df_missing, on=indexCols, how="left")
        .select(rowMissingInToSubtract)
        .filter(pl.col(rowMissingInToSubtract).is_null())
...
def filter_subtract_rows(
    ...
    *,
    ...
    df_pl = ensure_lazyframe(df)

    filterCols = []

    first_row_lf = df_pl.select(pl.col(c).first().alias(c) for c in indexCols)
    first_row = first_row_lf.limit(1).collect()
    first_values = {
    ...
    return ensure_polars_df(df_pl), filterCols
...
def duplicate_filter_rows_with_all(
    ...
    return result if use_lazy else ensure_polars_df(result)
...
def join_filtering_dataframe_and_rows_to_subtract(df, dfToSubtract, indexCols):
    ...
    joined = df.join(dfToSubtract, on=indexCols, how="inner")
...
def join_rows_to_subtract_to_main_df(df, dfFilter, paramDict):
    ...
    dfFilter = dfFilter.unique()
    df = df.join(dfFilter, on=loopRandomKey, how="left").unique()
    return df, paramDict
...
def subtract_values(
    ...
    for col in columns:
        if toSubtractStem in col:
            ...
            exprs.append(
                ...
                .then((pl.col(base) - pl.col(col).fill_null(0)).clip(lower_bound=0))
    ...
    df_rest = df_pl.slice(1).sort(randomKey)
    if use_lazy:
