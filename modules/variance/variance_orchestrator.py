...
def build_variance_calculation_array(aggregationsToPlot,chartDict):
    ...
    varianceArray=[]
    aggregationsToPlotDict={totalVarianceAggregation:["Price & units & mix"],
                            ...
                            costsUnitsAggregation:["Price on margin","Units & mix on margin","Cost","Indirect Costs","Balance"],
                            costsVolumeAggregation:["Price on margin","Volume & mix on margin","Cost","Indirect Costs","Balance"],
                            costsUnitsMixAggregation:["Price on margin","Units on margin","Mix on margin","Cost","Indirect Costs","Balance"],
                            costsVolumeMixAggregation:["Price on margin","Volume on margin","Mix on margin","Cost","Indirect Costs","Balance"],
                            marginUnitsRateAggregation:["Price on margin","Units & mix on margin","Margin rate","Indirect Costs","Balance"],
                            marginVolumeRateAggregation:["Price on margin","Volume & mix on margin","Margin rate","Indirect Costs","Balance"],
                            }                            
...
def select_variance_aggregations_to_plot(chartDict):
    ...
    salesAggregationsToPlot=[totalVarianceAggregation,priceAndUnitsAggregation,priceAndVolumeAggregation,mixAndUnitsAggregation,mixAndVolumeAggregation]
    marginAggregationsToPlot=[marginUnitsRateAggregation,costsUnitsAggregation,costsVolumeAggregation,costsUnitsMixAggregation,costsVolumeMixAggregation]
    aggregationsToPlot=[]
...
def find_unique_node_chains(dfCopy, indexCols, paramDict):
    ...
    dfNodes = (
        lfNodes.sort(group_byCols).with_columns(
            [
                pl.col(c)
...
def delete_duplicate_nodes(dfCopy, indexCols, valueCols, paramDict):
    ...
    dfResult = dfResult.unique(subset=indexCols, keep="first")
...
def process_index_combinations(dfCopy,resultArray,indexCols,valueCols,paramDict):
    ...
    if len(frameArray)>0:
        if use_lazy:
            frameArray = [
                f.lazy() if isinstance(f, pl.DataFrame) else f
                for f in frameArray
            ]
        dfResult = pl.concat(frameArray, how="diagonal", parallel=True)
        paramDict = check_if_duplicates_in_all_columns(
            dfResult, "Process index combinations", paramDict
        )
        dfResult = dfResult.unique(subset=indexCols)
...
def get_index_combinations(indexCols,nodeDict,paramDict):
    ...
    for x in range(1, arrayLength+1):
        resultArray=get_combinations_of_given_length(indexCols,x,nodeDict,resultArray) 
    resultArray.sort()  
...
def fill_parent_info(df, dfParentsCopy, nodeDict, paramDict):
    ...
    if deleteHierarchicalParents:
        ...
        lookups = [
            (child, parent, dfParentsCopy.select([child, parent]).unique())
        ...
        paramDict = check_if_duplicates_in_all_columns(df, "Fill parent info", paramDict)
...
def get_and_process_index_combinations(df,indexCols,valueCols,nodeDict,paramDict):
    ...
    df,paramDict=process_index_combinations(df,resultArray,indexCols,valueCols,paramDict) 
    return df,paramDict
...
def output_df_with_combinations(indexCols,nodeDict,df,valueCols,paramDict,chartDict):
    """
    grouping function together for order
    """  
    namingParams=get_naming_params()  
    ...
    countNumberOfNodesPerCombination=namingParams["countNumberOfNodesPerCombinationName"] 
    ...
    if len(indexCols)>1 and paramDict[namingParams["numberOfPeriodsFound"]]>1:
        ...
        df,paramDictCopy=get_and_process_index_combinations(df,indexCols,valueCols,nodeDict,paramDict)
        paramDict=get_data_sample(df,"get_and_process_index_combinations",False,paramDict)  
        ...
        dfSubtract,paramDict=make_df_copy_for_subtractions(df,indexCols,paramDict,chartDict)
        measure_time(dataPreparation,makeCopyForSubtraction,False) 
    else:
        dfSubtract=pl.DataFrame()
    return df,dfSubtract,indexCols,paramDict

def output_combinations_and_melt_results(indexCols,nodeDict,df,valueCols,paramDict,chartDict):
    """
    grouping function together for order
    """     
    df,dfSubtract,indexCols,paramDict=output_df_with_combinations(indexCols,nodeDict,df,valueCols,paramDict,chartDict)
    if  len(indexCols)>1:
      paramDict=get_data_sample(df,"output_df_with_combinations_df",False,paramDict) 
      paramDict=get_data_sample(df,"output_df_with_combinations_dfSubtract",False,paramDict) 
      df,paramDict=delete_duplicate_nodes_and_melt_result(df,dfSubtract,indexCols,valueCols,paramDict,chartDict)
...
def merge_dataframe_with_subtraction_df(df,dfSubtract,indexCols,paramDict):
    ...
    if isinstance(df, pl.LazyFrame):
        ...
        row_count = df.height
    statusMessage="Dataset has **"+ str("{:,.0f}".format(row_count)) +"** combinations among which the results will be drawn. "
    paramDict=add_status_message_to_paramDict(paramDict,statusMessage,0)
    if paramDict[simplifiedProcessing]:
        paramDict=add_status_message_to_paramDict(paramDict,simplifiedProcessingMessage,0)        
    else:
        use_lazy = isinstance(df, pl.LazyFrame)
        df_base = ensure_lazyframe(df)
        dfFilter = df_base.select(indexCols).with_columns(pl.lit(metConditionValue).alias(workColumn))
        dfSubtract = ensure_lazyframe(dfSubtract).join(dfFilter, on=indexCols, how="left")
        ...
        try:
            ...
            df_base = pl.concat([df_base, dfSubtract])
...
def delete_duplicate_nodes_and_melt_result(df,dfSubtract,indexCols,valueCols,paramDict,chartDict):
    """
    grouping function together for order
    """ 
    namingParams=get_naming_params() 
    if len(indexCols)>1 and paramDict[namingParams["numberOfPeriodsFound"]]>1:
          ...
          df=calculate_sales_mix_variance(df,paramDict,chartDict,indexCols)
          df,paramDict=calculate_margin_mix_variance(df,paramDict,chartDict,indexCols)
          paramDict=get_data_sample(df,"with_mix_variance_calculation",False,paramDict) 
          df,paramDict=melt_data_on_variance_cols(df,paramDict,chartDict,indexCols) 
          paramDict=get_data_sample(df,"melt_data_on_variance_cols",False,paramDict) 
          df,paramDict=change_nan_to_all(df,indexCols,paramDict) 
          df,paramDict=merge_dataframe_with_subtraction_df(df,dfSubtract,indexCols,paramDict)
    return df,paramDict
...
def drop_not_required_variance_columns(df,toRemoveCols,valueNameArray):
    ...
    for column in toRemoveCols:
        if column in valueNameArray:
            valueNameArray=take_filtered_value_out_of_option_list(valueNameArray,column)
...
def melt_data_on_variance_cols(df,paramDict,chartDict,indexCols):
    ...
    configParams=get_config_params()
    varianceAggregationParams=get_variance_aggregation_params()
    mixSalesAggregationArray=varianceAggregationParams[namingParams["mixSalesAggregationArray"]]
    mixMarginAggregationArray=varianceAggregationParams[namingParams["mixMarginAggregationArray"]]
    driverAggregationArray=varianceAggregationParams[namingParams["driverAggregationArray"]]
    ...
    percentAggregations=[marginVariancePercent,netOfDiscountVariancePercent,
                        cogsVariancePercent,volumeVariancePercent,
                        priceVariancePercent,costVariancePercent,marginRateVariancePercent]
    idVarArray=[]
    valueNameArray=[]
    ...
    toRemoveCols=[]         
    if chartDict[varianceAggregation] in [priceAndUnitsAggregation,priceAndVolumeAggregation,
                                          newAndLostUnitsAggregation,newAndLostVolumeAggregation,
                                          newAndLostUnitsMixAggregation,newAndLostVolumeMixAggregation,
                                          newAggregation,lostAggregation,changedAggregation,
                                          driverAndUnitsAggregation,driverAndVolumeAggregation,
                                          driverAggregation,
                                          unitPriceOnSalesAggregation,volumePriceOnSalesAggregation,
                                          unitsOnSalesAggregation,volumeOnSalesAggregation,
                                                ]+mixSalesAggregationArray:                                        
        toRemoveCols=[totalVariance,discountVariance,COGSVariance,marginRateVariance,costVariance]
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray) 
        if chartDict[varianceAggregation] in [unitPriceOnSalesAggregation,volumePriceOnSalesAggregation]: 
            toRemoveCols=[volumeVariance]
            df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray) 
        elif chartDict[varianceAggregation] in [unitsOnSalesAggregation,volumeOnSalesAggregation]: 
            ...
            df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)  
        elif chartDict[varianceAggregation] in [driverAndUnitsAggregation,driverAndVolumeAggregation]:
        ...
        elif chartDict[varianceAggregation] in [mixUnitsDriverAggregation,mixVolumeDriverAggregation]:
            if categoryWeightedDistributionName in columns:
                  df = df.rename({pureVolumeVariance: volumePerPosVariance})   
            else:
                  df = df.rename({pureVolumeVariance: averageTicketVariance}) 
    elif chartDict[varianceAggregation] in [totalVarianceAggregation,]:
        toRemoveCols=[priceVariance,volumeVariance,lostVolumeVariance,
                      newVolumeVariance,mixVariance,discountVariance,
                      COGSVariance,marginRateVariance,costVariance]         
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)            
    elif chartDict[varianceAggregation] in [
                                            discountsUnitsCogsAggregation,discountsVolumeCogsAggregation,
                                            discountsAggregation,cogsAggregation,
                                            unitPriceOnMarginAggregation,volumePriceOnMarginAggregation,
                                            unitsOnMarginAggregation,volumeOnMarginAggregation,
                                            ]:                                   
        toRemoveCols=[totalVariance,lostVolumeVariance,newVolumeVariance,
            mixVariance,marginRateVariance,costVariance]    
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)     
        if chartDict[varianceAggregation] in [discountsAggregation]:
            ...
            df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)
        elif chartDict[varianceAggregation] in [unitPriceOnMarginAggregation,volumePriceOnMarginAggregation,]: 
            toRemoveCols=[COGSVariance,volumeVariance,discountVariance]
            df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)
        elif chartDict[varianceAggregation] in [unitsOnMarginAggregation,volumeOnMarginAggregation]: 
            toRemoveCols=[priceVariance,COGSVariance,discountVariance]
            df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray) 
    elif chartDict[varianceAggregation] in [discountsAndUnitsAggregation,discountsAndVolumeAggregation,]:
        toRemoveCols=[totalVariance,lostVolumeVariance,newVolumeVariance,mixVariance,
                      COGSVariance,marginRateVariance]
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)                           
    elif chartDict[varianceAggregation] in [marginUnitsRateAggregation,marginVolumeRateAggregation]:
        toRemoveCols=[totalVariance,lostVolumeVariance,newVolumeVariance,mixVariance,discountVariance,
                      COGSVariance,costVariance]
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)              
    elif chartDict[varianceAggregation] in [costsUnitsAggregation,costsVolumeAggregation]:
        toRemoveCols=[totalVariance,lostVolumeVariance,newVolumeVariance,discountVariance,COGSVariance,marginRateVariance,mixVariance,]
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)  
    elif chartDict[varianceAggregation] in [costsUnitsMixAggregation,costsVolumeMixAggregation]:
        toRemoveCols=[totalVariance,lostVolumeVariance,newVolumeVariance,discountVariance,COGSVariance,marginRateVariance]
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)  
    elif chartDict[varianceAggregation] in [marginVarianceAggregation]:
        if chartDict[varianceInPercent] != metConditionValue:
            present_cols = [c for c in [totalVariance, discountVariance, COGSVariance] if c in columns]
            if present_cols:
                df = df.with_columns(
                    pl.sum_horizontal([pl.col(c) for c in present_cols]).alias(marginVariance)
                )
        elif chartDict[varianceInPercent] == metConditionValue :
            toRemoveCols=[netOfDiscountVariancePercent]
            df,paramDict=calculate_variance_in_percent(df,paramDict,chartDict,False)
        toRemoveCols=[totalVariance,priceVariance,volumeVariance,lostVolumeVariance,
                      newVolumeVariance,mixVariance,discountVariance,
                      COGSVariance,marginRateVariance,costVariance]+toRemoveCols                  
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)  
    elif chartDict[varianceAggregation] in [netOfDiscountAggregation] and discountName in columns:
        if chartDict[varianceInPercent] != metConditionValue:
            df = df.with_columns((pl.col(totalVariance) + pl.col(discountVariance)).alias(netOfDiscountVariance))            
        toRemoveCols=[totalVariance,priceVariance,volumeVariance,lostVolumeVariance,
                      newVolumeVariance,mixVariance,discountVariance,
                      COGSVariance,marginRateVariance,costVariance]+toRemoveCols
        df,valueNameArray=drop_not_required_variance_columns(df,toRemoveCols,valueNameArray)    
    ...
    df,paramDict=divide_back_if_multiplied(df,paramDict,chartDict,divideArray,False)  
    return df,paramDict            


def calculate_aggregated_price_volume_variance_value(df,paramDict,periodsArray,columns,chartDict):
    """
    ...
    if amountPeriodZero in columns:
        ...
        periodZeroSum = (
            df_temp.filter(pl.col(periodName) == periods[0])[amountName].sum()
        )
        periodOneSum = (
            df_temp.filter(pl.col(periodName) == periods[1])[amountName].sum()
        )
        varianceSum = periodOneSum - periodZeroSum
    ...
    paramDict[totalVarianceValueKey],paramDict[totalAmountPeriodZeroKey]=varianceSum,periodZeroSum   
    paramDict[totalAmountPeriodOneKey],paramDict[avgAmountPeriodsZeroOneKey]=periodOneSum,periodZeroOneAverage 
    ...
    if periodZeroOneAverage != 0:
      ...
      statusMessage=[totalVarianceValueKey+" "+varianceName,varianceValue,yoyPercentChangeValue]
      statusMessage=check_if_reverse_KPI_color(paramDict,statusMessage,amountName,periodZeroSum,periodOneSum)
...
def check_if_reverse_KPI_color(paramDict,statusMessage,metric,valuePeriodZero,valuePeriodOne):
    """
...
def calculate_aggregated_discount_variance_value(df,paramDict,periodsArray,columns):
    ...
    totalNetOfDiscountPeriodZeroinPercentKey=namingParams["totalNetOfDiscountPeriodZeroinPercent"]
    totalNetOfDiscountPeriodOneinPercentKey=namingParams["totalNetOfDiscountPeriodOneinPercent"]
    ...
    if discountVariance in columns:
        ...
        paramDict[firstPeriodDiscountsKey]=" 1st period: **"+firstPeriodDiscounts+"**."
        paramDict[secondPeriodDiscountsKey]=" 2nd period: **"+secondPeriodDiscounts+"**."
        averageNetDiscounts=(periodZeroNetOfDiscountSum+periodOneNetOfDiscountSum)/2
        ...
        if totalAmountPeriodZero != 0:
            periodZeroPercentMarginAfterDiscount=periodZeroNetOfDiscountSum/totalAmountPeriodZero*100
            paramDict[totalNetOfDiscountPeriodZeroinPercentKey]=periodZeroPercentMarginAfterDiscount 
        if totalAmountPeriodOne != 0:
            periodOnePercentMarginAfterDiscount=periodOneNetOfDiscountSum/totalAmountPeriodOne*100  
            paramDict[totalNetOfDiscountPeriodOneinPercentKey]=periodOnePercentMarginAfterDiscount
        percentMarginAfterdiscountUnitChange=periodOnePercentMarginAfterDiscount-periodZeroPercentMarginAfterDiscount
        paramDict[namingParams["percentVarianceAfterDiscounts"]]=percentMarginAfterdiscountUnitChange
        paramDict[totalNetOfDiscountPeriodZero],paramDict[totalNetOfDiscountPeriodOne]=periodZeroNetOfDiscountSum,periodOneNetOfDiscountSum    
        ...
        statusMessage=netOfDiscountVarianceKey+": **"+discountVarianceValue+"**. "
        if periodZeroNetOfDiscountSum != 0:
            yoyPercentChangeDelta=(periodOneNetOfDiscountSum*100/periodZeroNetOfDiscountSum)-100
            if abs(discountVarianceSum) > 100:
                yoyPercentChangeValue=str("{:,.1f}".format(round(yoyPercentChangeDelta,1)))
            else:
                yoyPercentChangeValue=str("{:,.1f}".format(round(percentMarginAfterdiscountUnitChange,1)))
        else:
            yoyPercentChangeValue=nothingThereString
        statusMessage=[netOfDiscountVarianceKey+" "+varianceName,discountVarianceValue,yoyPercentChangeValue]
        statusMessage=check_if_reverse_KPI_color(paramDict,statusMessage,netOfDiscountVarianceKey,periodZeroNetOfDiscountSum,periodOneNetOfDiscountSum)
        ...
        if abs(discountVarianceSum) > 100:
            printValue=str("{:,.1f}".format(round(periodZeroPercentMarginAfterDiscount,1)))
        else:
            printValue=str("{:,.1f}".format(round(periodZeroPercentMarginAfterDiscount,1)))     
        statusMessage1stB="1st period after discount as % of revenues: **"+printValue+"%**. "   
        if abs(discountVarianceSum) > 100:
            printValue=str("{:,.1f}".format(round(periodOnePercentMarginAfterDiscount,1)))
        else:
            printValue=str("{:,.1f}".format(round(periodOnePercentMarginAfterDiscount,1)))  
        statusMessage2ndB="2nd period after discount as % of revenues: **"+printValue+"%**. "      
        paramDict=add_status_message_to_paramDict(paramDict,statusMessage1stA+" "+statusMessage1stB,6)  
        paramDict=add_status_message_to_paramDict(paramDict,statusMessage2ndA+" "+statusMessage2ndB,6)  
    return paramDict
...
def calculate_aggregated_cogs_variance_value(df,paramDict,periodsArray,columns):
    ...
    if COGSVariance in columns:
        ...
        if totalAmountPeriodZero != 0:
            periodZeroPercentMarginAfterCOGS=periodZeroMarginSum/totalAmountPeriodZero*100
            paramDict[totalMarginPeriodZeroinPercent]=periodZeroPercentMarginAfterCOGS 
        if totalAmountPeriodOne != 0:
            periodOnePercentMarginAfterCOGS=periodOneMarginSum/totalAmountPeriodOne*100   
            paramDict[totalMarginPeriodOneinPercent]=periodOnePercentMarginAfterCOGS 
        percentMarginAftercogsUnitChange=periodOnePercentMarginAfterCOGS-periodZeroPercentMarginAfterCOGS 
        ...
        paramDict[totalMarginPeriodZeroKey],paramDict[totalMarginPeriodOneKey]=periodZeroMarginSum,periodOneMarginSum 
        ...
        if periodZeroMarginSum != 0:
            yoyPercentChangeDelta=(periodOneMarginSum*100/periodZeroMarginSum)-100    
            if abs(marginVarianceSum) > 100:
                yoyPercentChangeValue=str("{:,.1f}".format(round(yoyPercentChangeDelta,1)))
            else:
                yoyPercentChangeValue=str("{:,.1f}".format(round(yoyPercentChangeDelta,1)))
            ...
            yoyPercentChangeValue=nothingThereString
        statusMessage=[marginVarianceKey+" "+varianceName,marginVarianceValue,yoyPercentChangeValue]
        statusMessage=check_if_reverse_KPI_color(paramDict,statusMessage,marginName,periodZeroMarginSum,periodOneMarginSum)
        paramDict=add_status_message_to_paramDict(paramDict,statusMessage,3)
        if abs(marginVarianceSum) > 100:
        ...
        if abs(marginVarianceSum) > 100:
            printValue=str("{:,.1f}".format(round(periodZeroPercentMarginAfterCOGS,1)))
        else:
            printValue=str("{:,.1f}".format(round(periodZeroPercentMarginAfterCOGS,1)))     
        statusMessage1stB=" 1st period margin as % of revenues: **"+printValue+"%**."   
        if abs(marginVarianceSum) > 100:
        ...
        if abs(marginVarianceSum) > 100:
            printValue=str("{:,.1f}".format(round(periodOnePercentMarginAfterCOGS,1)))
        else:
            printValue=str("{:,.1f}".format(round(periodOnePercentMarginAfterCOGS,1)))    
        statusMessage2ndB=" 2nd period margin as % of revenues: **"+printValue+"%**. "   
        paramDict=add_status_message_to_paramDict(paramDict,statusMessage1stA+" "+statusMessage1stB,8)
        paramDict=add_status_message_to_paramDict(paramDict,statusMessage2ndA+" "+statusMessage2ndB,8)
    return paramDict

def calculate_aggregated_indirect_cost_variance_value(df,paramDict,periodsArray,columns):
    """
    if a discount columns exist, calculates the total variance after discount
    """ 
    namingParams=get_naming_params()    
    ...
    totalNetMarginPeriodZeroinPercent=namingParams["totalNetMarginPeriodZeroinPercent"]
    ...
    if indirectCostsColFound in paramDict and paramDict[indirectCostsColFound]:
      ...
      indirectCostsVarianceSum=periodOneIndirectCostsSum-periodZeroIndirectCostsSum
      paramDict[indirectCostsVariance]=indirectCostsVarianceSum
      if totalAmountPeriodZero != 0:
            periodZeroPercentMarginAfterIndCosts=periodZeroNetMarginSum/totalAmountPeriodZero*100
            paramDict[totalNetMarginPeriodZeroinPercent]=periodZeroPercentMarginAfterIndCosts
      if totalAmountPeriodOne != 0:
            periodOnePercentMarginAfterIndCosts=periodOneNetMarginSum/totalAmountPeriodOne*100 
            paramDict[totalNetMarginPeriodOneinPercent]=periodOnePercentMarginAfterIndCosts
      percentMarginAfterIndCostsChange=periodOnePercentMarginAfterIndCosts-periodZeroPercentMarginAfterIndCosts  
      paramDict[percentVarianceAfterIndCosts]=percentMarginAfterIndCostsChange 
      paramDict[netMarginVariance]=netMarginVarianceSum 
      paramDict[totalNetMarginPeriodZeroKey],paramDict[totalNetMarginPeriodOneKey]=periodZeroNetMarginSum,periodOneNetMarginSum         
      if abs(netMarginVarianceSum) > 100:
            netMarginVarianceValue=str("{:,.0f}".format(round(netMarginVarianceSum,0)))
      else:
            netMarginVarianceValue=str("{:,.1f}".format(round(netMarginVarianceSum,1)))
      if periodZeroNetMarginSum != 0:
            yoyPercentChangeDelta=(periodOneNetMarginSum*100/periodZeroNetMarginSum)-100    
            if abs(netMarginVarianceSum) > 100:
                yoyPercentChangeValue=str("{:,.1f}".format(round(yoyPercentChangeDelta,1)))
            else:
                yoyPercentChangeValue=str("{:,.1f}".format(round(yoyPercentChangeDelta,1)))
      ...
      statusMessage=[netMarginVariance+" "+varianceName,netMarginVarianceValue,yoyPercentChangeValue]
      statusMessage=check_if_reverse_KPI_color(paramDict,statusMessage,netMarginName,periodZeroNetMarginSum,periodOneNetMarginSum)
      paramDict=add_status_message_to_paramDict(paramDict,statusMessage,3)
      if abs(netMarginVarianceSum) > 100:
      ...
      if abs(netMarginVarianceSum) > 100:
            printValue=str("{:,.1f}".format(round(periodZeroPercentMarginAfterIndCosts,1)))
      else:
            printValue=str("{:,.1f}".format(round(periodZeroPercentMarginAfterIndCosts,1)))     
      statusMessage1stB="1st period after indirect costs as % of revenues: **"+printValue+"%**. "   
      if abs(netMarginVarianceSum) > 100:
      ...
      if abs(netMarginVarianceSum) > 100:
            printValue=str("{:,.1f}".format(round(periodOnePercentMarginAfterIndCosts,1)))
      else:
            printValue=str("{:,.1f}".format(round(periodOnePercentMarginAfterIndCosts,1)))     
      statusMessage2ndB="2nd period after indirect costs as % of revenues: **"+printValue+"%**. "
      paramDict=add_status_message_to_paramDict(paramDict,statusMessage1stA+" "+statusMessage1stB,10)    
      paramDict=add_status_message_to_paramDict(paramDict,statusMessage2ndA+" "+statusMessage2ndB,10) 
    return paramDict
...
def calculate_total_amounts(df,paramDict,chartDict):
    ...
    periodsArray=configParams["periodsArray"]
    columns,schema=get_schema_and_column_names(df)
    paramDict=calculate_aggregated_price_volume_variance_value(df,paramDict,periodsArray,columns,chartDict) 
    paramDict=calculate_aggregated_discount_variance_value(df,paramDict,periodsArray,columns)  
    paramDict=calculate_aggregated_cogs_variance_value(df,paramDict,periodsArray,columns)  
    paramDict=calculate_aggregated_indirect_cost_variance_value(df,paramDict,periodsArray,columns) 
    return paramDict                     
