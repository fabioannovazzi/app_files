...
description: Use when a user wants Claude to inspect a CSV/XLSX file, generate legacy scatter and bubble charts, small multiples, and relationship/outlier source data.
---
...
rendering path. Claude should interpret the prepared numbers, context JSON,
...
Treat `review_payload.json` as the bounded an external model provider-style payload for the local
