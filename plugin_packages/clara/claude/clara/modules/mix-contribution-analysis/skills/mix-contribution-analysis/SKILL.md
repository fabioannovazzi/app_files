...
description: Use when a user wants Claude to inspect a sales CSV/XLSX file, generate mix, composition, contribution, Mekko, Pareto, stacked, multitier and small-multiple charts, and interpret the patterns.
---
...
unresolved mapping decisions, deterministic run, Claude interpretation, and
delivery.
...
report, source pack, audit, and any `run_review.md` note.
...
bounded an external model provider-style payload for the local MCP widget. It contains contribution
...
8. If a separate run note is useful, write `run_review.md`; it is not a
client deliverable.
