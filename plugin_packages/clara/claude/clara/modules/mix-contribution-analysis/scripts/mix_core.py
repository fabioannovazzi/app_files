"""Mix and contribution analysis helpers for the Claude plugin."""
...
def build_summary_markdown(
    ...
    lines = (
        [
            "# Datos de origen del análisis de mix y contribución",
            "",
            f"- Archivo de origen: `{recipe.get('source_file')}`",
            f"- Métrica: `{contribution['metric']}`",
            f"- Total: `{contribution['total']:,.2f}`",
            f"- Dimensiones: `{', '.join(recipe['mappings'].get('dimensions') or [])}`",
            f"- Gráficos heredados generados: `{len(written)}`",
            f"- Candidatos con datos de gráficos heredados: `{len(data_written)}`",
            f"- Intentos de gráficos heredados no generados: `{len(failed)}`",
            "",
    ...
    lines.extend(
        [
            "",
            (
                "Los gráficos se generan mediante el flujo de gráficos heredado incluido. "
                "Los intentos que no produjeron una figura Plotly heredada figuran en la auditoría."
                if spanish
                else "Charts are generated by the vendored legacy charting pipeline. "
...
def write_client_report(
    ...
    charts_heading = "Gráficos" if spanish else "Charts"
