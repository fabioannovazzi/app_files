...
Claude agents remain responsible for every semantic mapping and review judgment.
"""
...
def _codex_agent(payload: Any, *, label: str) -> dict[str, Any]:
    if not isinstance(payload, dict) or payload.get("execution") != "codex_agent":
        raise MappingShardError(f"{label} must be attributed to a Claude agent")
...
def merge_mapping_review(
    ...
    *,
    ...
    """Merge reviewed slices under a separate independent Claude coordinator."""
