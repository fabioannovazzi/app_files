...
Do not call a product a fit merely because one attribute matches. Claude agents decide signal importance, gap meaning, candidate relevance, and narrative through the user's existing Claude plan. Deterministic code owns source rows, exact bundle matches, hashes, rendering, and correctness mechanics. Helper scripts make no separate model API call.
...
- Treat ranked reference candidates as leads for Claude evaluation, not automatic listing, launch, or assortment recommendations.

The files named as local remain on the user's machine, but their contents may
enter model context when Claude reads them for interpretation or review.
...
5. End with an **Artifact Card** listing local paths, purpose, the exact direct correctness verdict, independent-review state, unresolved items, and next action. `run_review.md` is the durable local card.

Default output policy: produce the richest natural private local package—transport receipts, pinned server evidence, accepted mapping-state and product-data snapshots, local image manifest, evidence catalog, scope metrics, report model, claim ledger, HTML draft, independent semantic review, desktop/mobile browser QA, final `report.html`, direct correctness verdict, `run_review.md`, and `final_artifacts.json`. These are not choices to propose. HTML is the production report format. Never edit plugin source or generated ZIPs during a user run, and never place generated ZIPs or run outputs in the plugin repository.
...
6. Have a Claude agent author `report_model.json` using `authoring_contract.json`:
...
8. Delegate `semantic_review.json` to a different Claude agent. The reviewer must cover every claim, every selected image (including explicit unavailable-image entries), and all required dimensions. The reviewer must not be the report author.
...
- `correctness_verdict.json`, `report.html`, `run_review.md`, and `final_artifacts.json`.
