...
description: Use when a user wants Claude to inspect a sales CSV/XLSX file, compare the current period with the previous-year period, generate year-over-year charts, small multiples, source packs, and interpret the movements.
---
...
dot/dumbbell, and year-over-year waterfall. The plugin is a guided Claude workflow: Claude inspects
...
JSON audit, diagnostics, charts, source packs, and Vera-written review files
...
6. For every completed run, create one client-ready Word deliverable: `period_comparison_client_report.docx`. Put Claude's business interpretation into that report and, if useful, mirror the same narrative in `period_comparison_client_report.md`. Use `run_review.md` only as an internal run note, not as a second client report. Never create competing Word outputs for the same run, and do not edit plugin source or generated ZIPs during a run.
...
Claude owns judgment: deciding ambiguous mappings with the user, interpreting
...
Plotly rendering path. Claude should interpret the prepared numbers, context
...
bounded an external model provider-style payload for the local MCP widget. It contains monthly
...
8. Review the generated structured outputs and update `period_comparison_client_report.md` / `period_comparison_client_report.docx` with Claude's business interpretation. If a separate run note is useful, write `run_review.md`; it is not a client deliverable.
...
Do not ask the user to edit JSON. Ask in business terms, then Claude updates the
...
- optional `period_comparison/run_review.md` for internal execution notes.
