...
def sort_dataframe_in_correct_order(
    ...
    if secondDimension in columns:
        ...
        df = df.with_columns(
            ...
            .then(pl.lit(globalAggregateOtherItemsName))
    ...
    if sortOnColumn:
        df = sort_utf8_lazy(df, secondDimension, descending=True)
    else:
...
def add_empty_rows_if_hierarchical(
    ...
    if numberOfRows > len(reversedList):
        ...
        for _ in range(rowsToAdd):
            new_rows.append(null_row_like(df, {columns[0]: invisibleCharacter}))
            invisibleCharacter += invisibleCharacter

        if new_rows:
            df = concat_aligned_lazy([df] + new_rows)
...
def add_empty_rows_if_not_hierarchical(
    ...
    columns, _ = get_schema_and_column_names(df)
    if secondDimension not in columns:
        df = df.with_columns(pl.lit(None).cast(pl.Utf8).alias(secondDimension))
        columns, _ = get_schema_and_column_names(df)

    new_rows: list[pl.LazyFrame] = []
    for item in reversedList:
        if item not in filteredUniqueItems:
            new_rows.append(null_row_like(df, {secondDimension: item}))
    ...
    if secondDimension in columns:
        ...
        df = sort_utf8_lazy(df, secondDimension)
    return df
...
def find_scaling_factor_for_overlay_metric(
    ...
    if count == 1 and metricsToPlot in chartDict and len(chartDict[metricsToPlot]) == 2:
        ...
        dfExport = concat_aligned_lazy(frameArray)
...
def add_average_to_stacked_bar(
    ...
    if yAxisDimension in chartDict and (
        ...
        if showAverageValue in chartDict and chartDict[showAverageValue]:
            ...
            average_exprs = []
            for col in columns:
                if col == chartDict[xAxisDimension]:
                    average_exprs.append(pl.lit(averageName).alias(col))
                elif is_numeric_dtype(schema[col]):
                    average_exprs.append(pl.col(col).mean().alias(col))
                else:
                    average_exprs.append(pl.lit(None).alias(col))
            dfAverage = lf.select(average_exprs)
...
def prepare_data_for_stacked_column(
    ...
    # 6) Create the legacy Value column expected by the drawing layer.
    columns, schema = get_schema_and_column_names(df)
    if column == totalName:
        if valueName not in columns:
            if totalName in columns:
                df = df.with_columns(pl.col(totalName).alias(valueName))
            else:
                numeric_cols = [
                    c
                    for c, dt in schema.items()
                    if c != xColumn and is_numeric_dtype(dt)
                ]
                df = df.with_columns(
                    pl.sum_horizontal([pl.col(c) for c in numeric_cols]).alias(
                        valueName
                    )
                )
    else:
    ...
    # 8) Verify all needed periods exist before deriving the title period range.
    df = check_if_all_periods_in_df(df, chartDict, paramDict)

    # 9) Grab first/last rows for the “most recent” vs. “least recent” period
    dfMostRecent = df.tail(1)
    mostRecentPeriod = df.select(pl.col("Period").max()).collect().item()
    leastRecentPeriod = df.select(pl.col("Period").min()).collect().item()
...
def prepare_data_for_stacked_bar_two_dimensions(
    ...
    lf = lf.sort(valueName)
    lf = rank_others_as_last(lf, aggregateOtherItemsNameKey, 0)
    lf = lf.select([xColumn] + sortedList + [valueName])
    lf = ensure_lazyframe(lf)
...
def _barmekko_sort_metric(
    lf: pl.LazyFrame,
    chartDict: dict,
    fallback_metric: str,
) -> str:
    """Prefer the displayed area metric for barmekko row ordering."""

    namingParams = get_naming_params()
    multipliedMetric = namingParams["multipliedMetric"]
    monetaryLocalCurrencyName = namingParams["monetaryLocalCurrencyName"]
    columns, schema = get_schema_and_column_names(lf)
    for metric in (chartDict.get(multipliedMetric), monetaryLocalCurrencyName):
        if (
            metric in columns
            and schema.get(metric) is not None
            and is_numeric_dtype(schema[metric])
        ):
            return str(metric)
...
def prepare_data_for_barmekko(
    ...
    group_byCols = [xColumn]
    metricsToPlot = list(dict.fromkeys([yAxisMetric, xAxisMetric]))
    ...
    sortMetric = _barmekko_sort_metric(lf, chartDict, yAxisMetric)
    sortKey = sortMetric
    if sortMetric not in metricsToPlot:
        sortKey = "__barmekko_sort_metric"
    ...
    aggregationExpressions = [pl.col(c).sum() for c in metricsToPlot]
    if sortKey != sortMetric:
        aggregationExpressions.append(pl.col(sortMetric).sum().alias(sortKey))
    df_grouped = lf.group_by(group_byCols).agg(aggregationExpressions)

    if not yColumn:
        selectCols = [xColumn, *metricsToPlot]
        if sortKey not in selectCols:
            selectCols.append(sortKey)
        lf_pivot = df_grouped.select(selectCols)
    else:
        lf_pivot = None
    ...
    lf_pivot = lf_pivot.sort(sortKey, descending=True)
    if sortKey != sortMetric:
        lf_pivot = lf_pivot.drop(sortKey)
