...
description: Use when a user wants Claude to inspect a CSV/XLSX file and produce deterministic Venn or UpSet overlap source data for items across sets such as retailers, brands, regions, channels, cohorts, or periods.
---
...
Use Cowork-native UI artifacts as part of a substantive run:
...
6. If a run note is useful, write `run_review.md`; it is not a client
...
- Claude owns mapping clarification and interpretation of the structured
  source data.
...
3. If validation succeeds, call `render_set_overlap_review` with the same
   payload objects so Claude can show the local HTML widget
