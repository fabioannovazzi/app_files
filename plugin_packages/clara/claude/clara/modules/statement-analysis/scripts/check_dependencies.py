"""Dependency checker for the statement-analysis plugin."""
...
REQUIRED_MODULES = ("csv", "json", "pathlib")
...
def requirement_name(line: str) -> str:
    """Return the importable package token from one requirements line."""
...
def import_name(package_name: str) -> str:
    """Translate a package name to its normal import name."""

    return package_name.replace("-", "_")


def modules_from_requirements(requirement_files: list[Path]) -> list[str]:
    """Return import names declared by explicit requirements files."""
...
def selected_requirement_files(explicit_files: list[str]) -> list[Path]:
    """Resolve requested requirements files under the plugin root."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Check runtime dependencies for the statement-analysis plugin."
    )
...
def main(argv: list[str] | None = None) -> int:
    """Check runtime imports and optionally write a JSON diagnostic."""

    args = parse_args(argv or sys.argv[1:])
    requirement_files = selected_requirement_files(args.requirements)
    if args.requirements and not requirement_files:
        sys.stdout.write("MISSING_REQUIREMENTS_FILE: no requested requirements file found\n")
        return 1
    required_modules = (
