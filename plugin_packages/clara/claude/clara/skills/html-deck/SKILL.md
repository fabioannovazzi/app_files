...
For revisions, use feedback already supplied in the connected folder or chat and follow the source-preserving revision mode in this skill.

State that the deck is ready to publish, not already published, unless an
authorized publishing step actually occurred. Create `run_review.md` only
