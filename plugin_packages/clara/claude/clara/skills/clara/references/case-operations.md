...
step. Wait for explicit approval. When approved, Claude must run the managed
...
notes Claude supplies. The helper itself does not browse or make model calls:
...
not manage an external model API key locally, launch the hosted voice service from the
...
workspace; advisory interpretation remains Claude work through the user's
Claude plan.
...
attribution, challenge, and semantic review as post-import Claude work over the
...
When Claude imports a voice bundle and the importer emits a speaker attribution
task, Claude must complete that task in the same workflow before using the
transcript for advisory or deck-revision work. Do not hand this back to the
user as a manual step unless the transcript is genuinely ambiguous after Claude
...
Claude/Clara text pass that preserves the unattributed source transcript.
...
`import_hosted_voice_bundle.py` only when the advisor or Claude needs to point at
...
`clara_review.md` created during import is the workspace for Claude/Clara's
semantic review.
...
is visual candidate evidence only; Clara/Claude still decides what the speaker
meant.
...
always Claude/Clara reasoning work using the attributed transcript, video/deck
...
Claude/model semantic pass. It still does not call a model and does not edit the
PPTX.

Before Claude/model writes the final change list, build focused interpretation
packets:
...
Claude/model then processes the packets, not the full workbench as one huge
...
is a semantic Claude/Clara interpretation step, not a deterministic
...
`deck_revision_execution_packets/`. Clara/Claude must execute one packet at a
...
deterministic application from changes that still need Claude/model editing,
...
then Clara/professional reviews them for relevance, sharpness, source diversity, and
...
`deck_revision_output_review.md` with the rendered-deck review checklist. Claude
...
this review may Claude complete it:
...
post-import Claude transcript-processing pass before using it as advisory evidence:
...
Speaker attribution is a text-only Claude/Clara loop after local import. The
transcript Claude reads may enter model context through the user's existing
Claude plan. The hosted server
...
`speaker_attribution_report.json`; that is not a stopping point for Claude. In
the same Claude/Clara workflow, Claude must read the task, raw transcript,
...
unavailable, Claude may use stable labels such as `Speaker 1` and `Speaker 2`.
...
When Claude/Clara later creates or replaces the attributed transcript with a
...
Claude must still inspect the transcript text and draft the semantic plan. The
...
Use `--browser chrome` for local microphone capture when the embedded Claude
...
pack when Claude should perform the second-pass advisory review of the full
...
5. Claude reads the source material and drafts the first bounded case-direction
...
When Claude drafts targeted follow-up questions separately from judgement
...
which Claude-structured statements are ready to rely on in the client pack. An
...
When the pending list is long, Claude/Clara should group entries into
advisor-readable inclusion bundles before showing the checklist. Bundle themes
are semantic judgement: Claude/Clara chooses labels such as decision area,
...
not show the advisor CLI commands as the normal workflow. Claude records the
...
Do not use Claude to merge conflicting judgement silently. After importing new
