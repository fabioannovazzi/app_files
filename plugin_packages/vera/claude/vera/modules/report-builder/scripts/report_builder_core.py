...
DOCX_COPY: dict[str, dict[str, str]] = {
    ...
    "draft": {
        "en": "Draft for review",
        "it": "Bozza da rivedere",
        "fr": "Projet à examiner",
        "de": "Entwurf zur Prüfung",
        "es": "Borrador para revisión",
    },
    "notes": {
        "en": "Notes",
        "it": "Note",
    ...
    "unit_currency": {
        "en": "Monetary amount",
        "it": "Importo monetario",
        "fr": "Montant monétaire",
        "de": "Geldbetrag",
        "es": "Importe monetario",
    },
    "unit_number": {
        "en": "Number",
        "it": "Numero",
    ...
    "audit_narrative": {
        "en": "Claude supplies the report commentary; the workflow calculates the reviewed figures.",
        "it": "Claude prepara i commenti del report; il workflow calcola le cifre verificate.",
        "fr": "Claude prépare les commentaires du rapport ; le workflow calcule les chiffres vérifiés.",
        "de": "Claude erstellt die Berichtskommentare; der Workflow berechnet die geprüften Zahlen.",
        "es": "Claude prepara los comentarios del informe; el workflow calcula las cifras verificadas.",
    },
    "audit_review": {
        "en": "Review missing sections and pending comments before final use.",
        "it": "Rivedere le sezioni mancanti e i commenti in sospeso prima dell’uso finale.",
        "fr": "Examiner les sections manquantes et les commentaires en attente avant utilisation finale.",
        "de": "Fehlende Abschnitte und ausstehende Kommentare vor der endgültigen Verwendung prüfen.",
        "es": "Revisar las secciones pendientes y los comentarios sin completar antes del uso final.",
    },
    "audit_numeric_pending": {
        "en": "Columns with numeric values stay outside the totals until their role as measures is reviewed.",
        "it": "Le colonne numeriche restano escluse dai totali finché non viene verificato il loro ruolo come misure.",
        "fr": "Les colonnes numériques restent exclues des totaux tant que leur rôle de mesure n’a pas été vérifié.",
        "de": "Numerische Spalten bleiben von den Summen ausgeschlossen, bis ihre Rolle als Messwerte geprüft wurde.",
        "es": "Las columnas numéricas quedan fuera de los totales hasta que se verifique su función como medidas.",
    },
}
...
def _reviewed_preview(
    ...
    """Expose only reviewed literal measures, retaining their physical cells."""

    header_idx = header_row - 1 if isinstance(header_row, int) else None
    display = rows_as_dicts(rows, header_idx)[: max(0, min(include_rows, 8))]
    display = [{key: row[key] for key in list(row)[:6]} for row in display]
    preview = _redact_numeric_preview_rows(display)
    if not decision or not preview:
        return preview, []
    physical_rows = [
        index + 1
        for index, row in enumerate(rows)
        if (header_idx is None or index > header_idx) and row_nonempty_count(row) > 0
    ][: len(preview)]
    offsets = {number: offset for offset, number in enumerate(physical_rows)}
    headers = list(preview[0])
    cells = []
    for column in decision["content"]["column_dispositions"]:
        name = column["column"]
        if name not in headers:
            continue
        for cell in column["cells"]:
            if cell["status"] == "excluded_by_review" and cell["row"] in offsets:
                preview[offsets[cell["row"]]][name] = "[excluded from calculation]"
            if cell["status"] != "included" or cell["row"] not in offsets:
                continue
            offset = offsets[cell["row"]]
            value = cell["scaled_value"]
            preview[offset][name] = value
            cells.append(
                {
                    "row_offset": offset,
                    "column_offset": headers.index(name),
                    "column": name,
                    "source_row": cell["row"],
...
def analysis_for_section(
    ...
    *,
    ...
    preview, preview_numeric_cells = _reviewed_preview(
        rows, effective_header_row, numeric_measure_decision, include_rows
    )
    return {
        "section": section_key,
        "title": clean_text(section_recipe.get("title")) or section_key,
        ...
        "preview_rows": preview,
        "preview_numeric_cells": preview_numeric_cells,
        "codex_comment": clean_text(section_recipe.get("codex_comment")),
    }
...
def render_markdown(recipe: dict[str, Any], analysis: dict[str, Any]) -> str:
    ...
    for section in analysis["sections"]:
        if section["status"] != "assigned" and not recipe.get("render", {}).get(
            "include_unassigned_tables", False
        ):
            continue
        ...
        if numeric_columns:
            ...
            for column in numeric_columns:
                ...
                lines.append(
                    ...
                    f"{display_numeric_unit(column.get('unit'), language)} | "
                    f"{docx_label('scale', language)}: "
                    f"{clean_text(column.get('scale'))}"
                )
        preview_table = markdown_table(
            display_preview(section.get("preview_rows") or [], language)
        )
...
def display_numeric_unit(unit: object, language: str) -> str:
    """Translate a declared unit, preserving unknown supplied labels verbatim."""
    value = clean_text(unit)
    return docx_label(f"unit_{value}", language) if value in NUMERIC_UNITS else value


def display_preview(
    rows: Sequence[dict[str, str]], language: str
) -> list[dict[str, str]]:
    """Localize mechanical withholding markers without changing source values."""
    labels = {
        "[numeric source value withheld]": docx_label("preview_unreviewed", language),
        "[excluded from calculation]": docx_label("preview_excluded", language),
    }
    return [
        {key: labels.get(value, value) for key, value in row.items()} for row in rows
    ]


def audit_notes(language: str, *, numeric_measure_pending: bool) -> list[str]:
    """Use the same localized audit explanations across all native build routes."""
    keys = ["audit_narrative", "audit_review"]
    if numeric_measure_pending:
        keys.append("audit_numeric_pending")
    return [docx_label(key, language) for key in keys]
...
def add_key_value_table(document: Any, rows: Sequence[tuple[str, Any]]) -> None:
    ...
    for label, value in rows:
        ...
        for paragraph in cells[0].paragraphs:
            set_paragraph_font(paragraph, bold=True, color="000000")
    if len(rows) <= 5:
    ...
    *,
    ...
    for idx, header in enumerate(headers):
        ...
        for paragraph in header_cells[idx].paragraphs:
            set_paragraph_font(paragraph, size=8.5, bold=True, color="000000")
...
def write_report_docx(
    ...
    add_key_value_table(
        ...
        (
            ...
            (
                docx_label("report_status", language),
                (
                    docx_label("draft", language)
                    if audit.get("status", "draft") == "draft"
                    else audit["status"]
                ),
            ),
        ),
    ...
    for section_analysis in analysis.get("sections", []):
        if section_analysis.get("status") != "assigned" and not recipe.get(
            "render", {}
        ).get("include_unassigned_tables", False):
            continue
    ...
    if missing_sections:
        ...
        for missing in missing_sections:
            document.add_paragraph(
                section_title(clean_text(missing), language), style="List Bullet"
            )
...
def _preview_evidence_rows(analysis: dict[str, Any]) -> list[dict[str, Any]]:
    """Bind each displayed detail to the same reviewed source as its total."""

    totals = {
        (row["section_index"], row["column"]): row
        for row in _numeric_evidence_rows(analysis)
    }
    evidence = []
    for section_index, section in enumerate(analysis["sections"]):
        for cell in section.get("preview_numeric_cells", []):
            total = totals[(section_index, cell["column"])]
            pointer_key = cell["column"].replace("~", "~0").replace("/", "~1")
            evidence.append(
                {
                    **total,
                    "evidence_id": total["evidence_id"].removesuffix(".sum")
                    + f".row_{cell['source_row']:03d}",
                    "value": cell["value"],
                    "numeric_rows": [cell["source_row"]],
                    "numeric_count": 1,
                    "source_locator": total["source_locator"].split("!")[0]
                    + "!"
                    + cell["source_coordinate"],
                    "prepared_locator": f"/sections/{section_index}/preview_rows/"
                    + f"{cell['row_offset']}/{pointer_key}",
                    "preview_row_offset": cell["row_offset"],
                    "preview_column_offset": cell["column_offset"],
                }
            )
    return evidence


def _preview_numeric_locations(
    output_dir: Path,
    analysis: dict[str, Any],
    evidence_rows: Sequence[dict[str, Any]],
) -> dict[str, list[dict[str, str]]]:
    """Reopen every detail table and reject altered, omitted or moved values."""

    if not evidence_rows:
        return {}
    workbook = openpyxl.load_workbook(
        output_dir / "report_tables.xlsx", data_only=False, read_only=True
    )
    document = Document(output_dir / "report.docx")
    markdown = (output_dir / "report_draft.md").read_text(encoding="utf-8")
    rendered_previews = analysis.get("include_table_previews", True)
    ...
    try:
        for section_index, section in enumerate(analysis["sections"]):
            ...
            sheet_offset += 1
            headers = list(rows[0])
            expected = [headers, *[[row[key] for key in headers] for row in rows]]
            sheet = workbook.worksheets[sheet_offset]
            actual = [
                [clean_text(cell.value) for cell in row] for row in sheet.iter_rows()
            ]
            if actual != expected or any(
                cell.data_type == "f" for row in sheet.iter_rows() for cell in row
            ):
                raise ValueError("Numeric detail closure failed for Excel preview")
            table_index = -1
            markdown_start = -1
            if rendered_previews:
                display_rows = display_preview(
                    rows, str(analysis.get("language") or "en")
                )
                display_expected = [
                    headers,
                    *[[row[key] for key in headers] for row in display_rows],
                ]
                table_index = next(
                    (
                        index
                        for index in range(table_cursor, len(document.tables))
                        if [cell.text for cell in document.tables[index].rows[0].cells]
                        == headers
                    ),
                    -1,
                )
                if (
                    ...
                    raise ValueError("Numeric detail closure failed for Word preview")
                table_cursor = table_index + 1
                rendered_table = markdown_table(display_rows)
                markdown_start = markdown.find(rendered_table, markdown_cursor)
                if markdown_start < 0:
                    raise ValueError(
                        "Numeric detail closure failed for Markdown preview"
                    )
                markdown_cursor = markdown_start + len(rendered_table)
            for evidence in evidence_rows:
                if evidence["section_index"] != section_index:
                    continue
                row_index = evidence["preview_row_offset"] + 2
                column_index = evidence["preview_column_offset"] + 1
                value = evidence["value"]
                if expected[row_index - 1][column_index - 1] != value:
                    raise ValueError("Numeric detail closure failed for prepared value")
                outputs = [
                    {
                        "artifact_ref": "output.report_tables",
                        "locator": f"{sheet.title}!{_excel_column_name(column_index)}{row_index}",
                        "value": value,
                    }
                ]
                if rendered_previews:
                        ...
                        [
                            {
                                ...
                                "locator": f"table:{table_index + 1}/row:{row_index}/cell:{column_index}",
                                "value": value,
                            },
                            {
                                "artifact_ref": "output.report_draft",
                                "locator": f"line:{markdown[:markdown_start].count(chr(10)) + row_index + 1}/column:{column_index}",
                                "value": value,
                            },
                        ]
                    )
                locations[evidence["evidence_id"]] = outputs
    finally:
        workbook.close()
    return locations
    ...
    *,
    ...
    for evidence in evidence_rows:
        expected = (
            ...
            f"{docx_label('unit', language)}: {display_numeric_unit(evidence['unit'], language)} | "
...
def write_numeric_evidence_ledger(
    ...
    *,
    ...
    preview_locations = _preview_numeric_locations(
        output_dir, prepared, preview_evidence
    )
    entries = []
    for evidence in evidence_rows:
    ...
    for evidence in preview_evidence:
        source_value = _replay_source_numeric_value(
            evidence, sources_by_id, source_resolution_dir
        )
        entries.append(
            {
                "evidence_id": evidence["evidence_id"],
                ...
                "outputs": preview_locations[evidence["evidence_id"]],
                "calculation_ref": "calculation.reviewed_signed_scaled_cell_sum.v2",
                "decision_ref": evidence["decision_ref"],
                "limitations": evidence["limitations"],
            }
...
def _build_report_in_place(
    ...
    *,
    ...
    rendered_audit_notes = audit_notes(
        report_language, numeric_measure_pending=bool(numeric_measure_pending_sections)
    )
    audit = {
        "version": 1,
        "status": "draft",
