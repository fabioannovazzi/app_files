"""Deterministic core for Vera Startup Business Plan.
...
WORKFLOW_ID = "startup-business-plan"
CASE_SCHEMA = "vera.startup_business_plan_case.v1"
PLAN_SCHEMA = "vera.startup_business_plan.v1"
COMMENTARY_SCHEMA = "vera.startup_business_plan_commentary.v1"
MAX_PERIODS = 60
MAX_SCENARIOS = 8
...
def validate_case(case: Mapping[str, Any]) -> None:
    ...
    _text(case["venture_stage"], label="case.venture_stage", maximum=300)
...
def build_business_plan(case: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "venture_stage": case["venture_stage"],
...
def build_model_context(plan: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vera.startup_business_plan_model_context.v1",
        "workflow_id": WORKFLOW_ID,
        "case_id": plan["case_id"],
        "entity_name": plan["entity_name"],
        "venture_stage": plan["venture_stage"],
...
def write_excel(path: Path, plan: Mapping[str, Any]) -> None:
    ...
    review_rows = (
        ...
        ("Venture stage", plan["venture_stage"]),
        ("Audience", plan["audience"]),
...
def render_markdown(plan: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Venture stage: {plan['venture_stage']}",
...
<body><main>
  <p>Vera · Startup Business Plan</p>
  <h1>{html.escape(str(plan['entity_name']))}</h1>
  <div class="meta"><span>Venture stage: {html.escape(str(plan['venture_stage']))}</span><span>Status: {html.escape(str(plan['status']))}</span><span>Review: {html.escape(str(plan['review_status']))}</span><span>Currency: {html.escape(str(plan['reporting_currency']))}</span></div>
