...
def _codex_home_boundary_inputs() -> dict[str, Any]:
    ...
    if codex_home.is_symlink() or not codex_home.is_dir():
        raise ValueError("Claude home must be an ordinary directory")
...
def _resolved_codex_binary(codex_bin: Path | None) -> Path:
    ...
    try:
        ...
        raise ValueError("Claude CLI path cannot be resolved") from exc
...
def _qualification_canaries(
    *,
    ...
    try:
        ...
        raise ValueError("Claude version output is not UTF-8") from exc
    if version_result["return_code"] != 0 or version != PINNED_CODEX_VERSION:
        raise ValueError("Claude version did not match the qualified worker")
    return {
...
def _worker_banner_attestation(
    ...
    *,
    ...
    """Record any Claude CLI worker configuration banner without overstating it.

    The banner is a CLI self-report rather than a provider-signed attestation.
    JSONL mode in Claude CLI 0.148 does not currently emit the normal banner.
    ...
    if model_reports and set(model_reports) != {model}:
        raise ValueError("Claude worker reported another model")
    if effort_reports and set(effort_reports) != {reasoning_effort}:
        raise ValueError("Claude worker reported another reasoning effort")
    return {
...
def run_isolated_luna_worker(
    *,
    ...
    This is the reusable native-Claude boundary. Domain-specific callers own
    ...
    try:
        ...
        if _codex_home_boundary_inputs()["bindings"] != boundary_inputs["bindings"]:
            raise ValueError("Claude boundary inputs changed while Luna was running")
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise ValueError("Claude CLI changed while Luna was running")
        if (
            _stable_executable_binding(
                SANDBOX_EXEC_PATH,
                label="macOS sandbox-exec",
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if event_type in {"item.started", "item.updated", "item.completed"}:
            if not turn_started and line_number == 2 and not turn_completed:
                # Claude CLI 0.148 may begin the turn implicitly with its first
...
def _validate_launch_receipt(
    ...
    *,
    ...
    if not isinstance(codex_path, str) or not Path(codex_path).is_absolute():
        raise ValueError("Luna launch Claude path is invalid")
    expected_boundary = {
        "contract_id": WORKER_BOUNDARY_CONTRACT_ID,
        "platform": "Darwin",
    ...
    _non_negative_int(boundary["codex_bytes"], label="Luna Claude byte count")
