...
general angle confirmation. When `required` is true, Claude must not draft until
...
for legal-framework confirmation. When `required` is true, Claude must not draft until the
