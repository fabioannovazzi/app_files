...
TOOL_VALIDATE = "validate_open_item_reconciliation_review"
TOOL_RENDER = "render_open_item_reconciliation_review"
TOOL_SAVE = "save_open_item_reconciliation_decisions"
TOOL_APPLY = "apply_open_item_reconciliation_decisions"
MAX_ITEMS = 2500
...
def _save_decisions_in_place(
    ...
    return {
        ...
        "message": (
            f"Saved {ui_decisions['decision_count']} Open-item Reconciliation decisions."
        ),
    ...
    trace.append(
        {
            ...
            "command": ["open-item-reconciliation-review-server", "apply_decisions"],
...
def _apply_decisions_in_place(
    ...
    return {
        ...
        "message": (
            ...
            "Open-item Reconciliation decisions."
        ),
...
def _widget_html(output_dir: Path) -> str:
    ...
    (function () {{
      ...
      const stateKey = `open-item-reconciliation:${{serverPayload.review_payload?.run_id || "run"}}`;
      function readState() {{
...
def _tool_result(output_dir: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == TOOL_VALIDATE:
        ...
        return {
            ...
            "message": "Open-item Reconciliation review payload is valid.",
            "review_payload": session["review_payload"],
        }
    if name == TOOL_RENDER:
    ...
    raise ValueError(f"unknown Open-item Reconciliation widget tool: {name}")
...
def serve_review(
    ...
    *,
    ...
    LOGGER.info("Open-item Reconciliation review server: %s", url)
    ...
    try:
        ...
        LOGGER.info("Stopping Open-item Reconciliation review server")
    finally:
        httpd.server_close()
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Open the Open-item Reconciliation review UI in a local browser and "
