...
The returned `workflow_contract` binds `plugin_id: vera`, the workflow ID, the
absolute Vera `plugin_root` and its `skill_path`. Read that exact skill and keep
the step within its Vera contract. Never resolve a matching name from another
