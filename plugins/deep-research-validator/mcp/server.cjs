...
const SERVER_NAME = "deep-research-validator-widgets";
...
const WIDGET_URI = "ui://widget/deep-research-review.html";
...
const TOOL_NAMES = {
  ...
  saveDecisions: "save_deep_research_decisions",
  applyDecisions: "apply_deep_research_decisions",
};
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "deep-research-validator") {
    throw new Error('review_payload.plugin must be "deep-research-validator"');
  }
  ...
  const payload = {
    widget_type: "deep_research_review",
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "deep_research_decisions",
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "deep_research"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "deep_research_application",
