...
const SERVER_NAME = "mix-contribution-widgets";
...
const WIDGET_URI = "ui://widget/mix-contribution-review.html";
const WIDGET_MIME_TYPE = "text/html;profile=mcp-app";
const MAX_ITEMS = 2500;
const MAX_PAYLOAD_BYTES = 2_000_000;
const TOOL_NAMES = {
  validateReview: "validate_mix_contribution_review",
  renderReview: "render_mix_contribution_review",
  saveDecisions: "save_mix_contribution_decisions",
  applyDecisions: "apply_mix_contribution_decisions",
};
...
const ACTION_STATUSES = {
  ...
  reject: "rejected",
  edit: "edited",
  mark_unclear: "needs_source_data",
  request_more_documents: "needs_source_data",
  skip: "skipped",
};
...
function responseCopy(inputArgs) {
  if (requestLanguage(inputArgs) !== "es") {
    return {
      saved: (count) => `Saved ${count} Mix Contribution decisions.`,
      saveWithoutPath:
        "Validated decisions. No run_intake.output_dir was provided, so nothing was written.",
      applied: (count) => `Applied ${count} Mix Contribution decisions.`,
      applyWithoutPath:
        "Validated applied decisions. No run_intake.output_dir was provided, so nothing was written.",
      valid:
        "Mix Contribution review payload is valid. It is safe to call render_mix_contribution_review once.",
      instructions:
        "Use validate_mix_contribution_review before render_mix_contribution_review. Prefer the MCP widget for Mix Contribution review handoff; use save_mix_contribution_decisions to persist reviewer actions to ui_decisions.json and apply_mix_contribution_decisions to write applied_decisions.json plus final_artifacts.json status when decisions are collected; fall back to Markdown/static review only when MCP is unavailable.",
    };
  }
  return {
    saved: (count) => `Se han guardado ${count} decisiones de la revisión de contribución al mix.`,
    saveWithoutPath:
      "Las decisiones son válidas. No se ha indicado run_intake.output_dir, por lo que no se ha escrito ningún archivo.",
    applied: (count) => `Se han aplicado ${count} decisiones de la revisión de contribución al mix.`,
    applyWithoutPath:
      "Las decisiones aplicadas son válidas. No se ha indicado run_intake.output_dir, por lo que no se ha escrito ningún archivo.",
    valid:
      "Los datos de revisión de contribución al mix son válidos. Ya puede llamar una vez a render_mix_contribution_review.",
    instructions:
      "Use validate_mix_contribution_review antes de render_mix_contribution_review. Utilice preferentemente el widget MCP para entregar la revisión; use save_mix_contribution_decisions para guardar las decisiones en ui_decisions.json y apply_mix_contribution_decisions para escribir applied_decisions.json y actualizar el estado de final_artifacts.json. Recurra a la revisión Markdown o estática solo si MCP no está disponible.",
  };
}
...
function toolUiMeta(resourceUri, toolName = null, language = "en") {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] =
      language === "es" ? "Abriendo la revisión de contribución al mix" : "Rendering Mix Contribution review";
    meta["openai/toolInvocation/invoked"] =
      language === "es" ? "Revisión de contribución al mix abierta" : "Rendered Mix Contribution review";
  }
  return meta;
}
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Mix Contribution review surface for contribution drivers, generated charts, source contexts, follow-up requests, and final artifacts.",
  ...
  return [
    {
      ...
      title: "Validate Mix Contribution review payload",
      description:
        "Validate the Mix Contribution review-session payload before rendering. Call this first, then render_mix_contribution_review.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Mix Contribution review",
      description:
        "Render a Mix Contribution review-session payload as an MCP HTML widget for contribution drivers, charts, source contexts, follow-ups, and artifacts.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Mix Contribution review decisions",
      description:
        "Validate Mix Contribution review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Mix Contribution review decisions",
      description:
        "Validate Mix Contribution review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "mix_contribution_review_widget",
      title: "Mix Contribution review widget",
      description:
        "Renders Mix Contribution review-session payloads with searchable contribution drivers, chart artifacts, contexts, and follow-up requests.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
    throw new Error(`unknown Mix Contribution widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "mix-contribution-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "mix-contribution-analysis") {
    throw new Error('review_payload.plugin must be "mix-contribution-analysis"');
  }
  ...
  const payload = {
    widget_type: "mix_contribution_review",
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`Mix Contribution widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function normalizeDecision(decision, itemById, seenIds, decidedAt, index) {
  ...
  // Missing-document requests copy only explicit review metadata; no semantic source inference happens here.
  ...
  // Follow-up context copies only explicit item/source metadata to make blocker queues actionable.
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "mix_contribution_decisions",
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "mix_contribution"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function requestedDocumentsFromReviewContext(decision, item, data) {
  ...
  const sourceRecords = Array.isArray(item.source_records) ? item.source_records : [];
...
function followupContextFromReviewContext(decision, item, data) {
  ...
  const records = [
    data,
    ...(Array.isArray(item.source_records) ? item.source_records.filter(isPlainObject) : []),
  ];
  const fields = [
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "mix_contribution_application",
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    return {
      ok: true,
      validation_type: "mix_contribution_review",
  ...
  throw new Error(`unknown Mix Contribution widget tool: ${name}`);
}
