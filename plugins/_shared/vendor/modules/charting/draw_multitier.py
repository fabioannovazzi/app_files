...
def _lock_multitier_small_multiple_y_slots(
    fig: go.Figure,
    ...
    """Keep padded small-multiple bar rows packed at the top of each panel."""
    ...
    categories = [item for item in categories if item is not None]
    if not categories:
        return fig

    invisibleCharacter = namingParams["invisibleCharacter"]
    visibleCategories = [
        item for item in categories if not str(item).startswith(invisibleCharacter)
    ]
    visibleSlotCount = max(len(visibleCategories), 1)
    totalSlotCount = len(categories)

    if visibleSlotCount < totalSlotCount:
        yaxes = list(fig.select_yaxes(row=row, col=col))
        if yaxes:
            domain = yaxes[0].domain
            if domain:
                domainStart, domainEnd = float(domain[0]), float(domain[1])
                slotRatio = visibleSlotCount / totalSlotCount
                fig.update_yaxes(
                    domain=[
                        domainEnd - ((domainEnd - domainStart) * slotRatio),
                        domainEnd,
                    ],
                    row=row,
                    col=col,
    ...
    fig.update_yaxes(
        ...
        range=[visibleSlotCount - 0.5, -0.9],
        row=row,
        col=col,
    )
    return fig
...
def _sort_multitier_small_multiple_rows(
    df: pl.DataFrame | pl.LazyFrame,
    column: str,
    periodOrder: list[str],
    chartDict: dict,
) -> pl.LazyFrame:
    """Sort small-multiple multi-tier rows by current value before drawing."""
    ...
    if (
        plotSmallMultiplesKey not in chartDict
        or not chartDict[plotSmallMultiplesKey]
        or chosenChart != multitierBarChart
    ):
        return utils.ensure_lazyframe(df)
    ...
    sortMetric = next((item for item in reversed(periodOrder) if item in columns), None)
    if sortMetric is None:
        return lf
    ...
    return (
        lf.with_columns(
            ...
            pl.col(sortMetric).fill_null(0).alias("__sort_value"),
        )
        .with_columns(
            pl.col(column)
            .str.starts_with(invisibleCharacter)
            .fill_null(True)
            .alias("__is_padding"),
            pl.col(column)
            .str.contains(aggregateOtherItemsName, literal=True)
            .fill_null(False)
            .alias("__is_other"),
        )
        .sort(
            ["__is_padding", "__is_other", "__sort_value", column],
            descending=[False, False, True, False],
        )
        .drop(["__is_padding", "__is_other", "__sort_value"])
    )


def _dimension_panel_param_dict(paramDict: dict) -> dict:
    """Return per-panel parameters so different dimensions rank independently."""

    namingParams = get_naming_params()
    globalUniqueItemsArrayKey = namingParams["globalUniqueItemsArray"]
    panelParamDict = copy.deepcopy(paramDict)
    panelParamDict[globalUniqueItemsArrayKey] = []
    return panelParamDict
...
def draw_multitier_column_chart(dfCopy,chosenDimension,metricArray,repeatArray,paramDict,chartDict):
    ...
    if is_valid_lazyframe(dfCopy):
        ...
        if plotSmallMultiplesKey in chartDict and chartDict[plotSmallMultiplesKey]:
            repeatArrayToPlot = _rank_repeat_array_by_size(
                dfCopy, repeatArrayToPlot, chosenDimension, metricArray
            )
            repeatArray = repeatArrayToPlot
        if plotSmallMultiplesKey in chartDict and chartDict[plotSmallMultiplesKey]:
...
def add_legends_to_multitier_bar(
    ...
    try:
        ...
        vals = lf.select(exprs).collect(engine="streaming")
        val_columns, _ = get_schema_and_column_names(vals)

        def val(name: str):
            if name not in val_columns or vals[name][0] is None:
                return 0
            return vals[name][0]
...
def add_annotations_to_multitier_bar_chart(fig,df,uniqueItemsNumber,uniqueItems,paramDict,chartDict,column,metric,row,col):
    ...
    if compareScenariosOrPeriods in chartDict and chartDict[compareScenariosOrPeriods]==compareScenarios:
        ...
        fcName=None  
    offset = -.15        
    df = _sort_multitier_small_multiple_rows(df, column, periodOrder, chartDict)
    ...
    fig = _lock_multitier_small_multiple_y_slots(fig, df, column, row, col, chartDict)
    return fig,chartDict
        ...
        if is_valid_lazyframe(df):
            if plotSmallMultiplesKey not in chartDict or not chartDict[plotSmallMultiplesKey]:
                ...
                for column in columnsToPlotNoTotal:
                    ...
                    panelParamDict = _dimension_panel_param_dict(paramDict)
                    df1,uniqueItems,panelParamDict=prepare_data_for_multitier_bar_plot(df1,column,xColumn,metric,valueCols,chartDict,panelParamDict,"X")
                    dfDim=duplicate_dataframe(df1)
                    ...
                    fig,chartDict=add_annotations_to_multitier_bar_chart(fig,df1,maxItems+1,uniqueItems,panelParamDict,chartDict,column,metric,countRows,countCols)
