...
FOCUS_ITEM_HIGHLIGHT_COLOR = "#0065FF"
FOCUS_ITEM_HIGHLIGHT_MAX_ITEMS = 1
...
def insert_highlight_color(column,rankedArray,colorArray,paramDict,chartDict):
    ...
    if column != totalName or not column:
        if highlightedDimension in chartDict and len(chartDict[highlightedDimension])>0:
            ...
            if isinstance(highlightedDimensionArray, str):
                highlightedDimensionArray = [highlightedDimensionArray]
            highlightedDimensionArray = list(highlightedDimensionArray)[
...
def SetColorRedToGreen(x):
    colorDict={
                  ...
                  "blueColor":FOCUS_ITEM_HIGHLIGHT_COLOR,
          }
    if x is None:
        return colorDict["greyColor"]
    if(x < 1):
...
def get_hightlight_color(chartDict,colorDict):
    return FOCUS_ITEM_HIGHLIGHT_COLOR
...
def add_workcolumns_to_millify(
    ...
    # Now apply the expression and make the label helper cast-safe.
    df = df.with_columns(expr.alias(workColumnThree))
    df = df.with_columns(
        pl.when(pl.col(workColumnThree).is_finite())
        .then(pl.col(workColumnThree))
        .otherwise(pl.lit(0.0))
        .fill_null(0.0)
...
def get_number_prefix(
    lf: float | int | pl.DataFrame | pl.LazyFrame,
    value_col: str | dict,
    chartDict: dict | None,
    decimals: int | bool,
    ...
    if not isinstance(lf, (pl.DataFrame, pl.LazyFrame)):
        max_val = abs(float(lf or 0))
        prefix = next(
            (
                candidate
                for candidate, bound in mill_bounds.items()
                if bound and max_val > bound
            ),
            "",
        )
        chart_dict = value_col if isinstance(value_col, dict) else {}
        decimals_out = decimals if type(decimals) is int else 1
        chart_dict[valuePrefixName] = prefix
        if metric:
            chart_dict[valuePrefixMetric] = metric
            chart_dict.setdefault(valuePrefixDict, {})[metric] = prefix
        return prefix, chart_dict, decimals_out

    lf = lf.lazy() if isinstance(lf, pl.DataFrame) else lf
    chartDict = chartDict or {}
...
def get_correct_multiplier(
    ...
    valuePrefixMetric        = n["valuePrefixMetric"]
    valuePrefixDict          = n["valuePrefixDict"]
    ...
    source_metric = chartDict.get(valuePrefixMetric)
    source_prefix = ""
    if valuePrefixDict in chartDict and source_metric in chartDict[valuePrefixDict]:
        source_prefix = chartDict[valuePrefixDict][source_metric]
    metric_prefix_found = False
    metric_prefix = ""
    if metric and valuePrefixDict in chartDict and metric in chartDict[valuePrefixDict]:
        metric_prefix_found = True
        metric_prefix = chartDict[valuePrefixDict][metric]
    if chartDict.get(n["plotSmallMultiplesOtherCharts"], False) and metric_prefix_found:
        chartDict[valuePrefixName] = metric_prefix
        chartDict[valuePrefixMetric] = metric
        if IBCSdecimalName in chartDict and chartDict[IBCSdecimalName] >= 0:
            decimals = chartDict[IBCSdecimalName]
        return metric_prefix, chartDict, decimals
    if (
        source_prefix
        and chosenChart
        in [
            n["multitierColumnChart"],
            n["trendComparisonChart"],
            n["trendComparisonByPeriodChart"],
            n["horizontalWaterfallChart"],
        ]
        and metric
        ...
        if IBCSdecimalName in chartDict and chartDict[IBCSdecimalName] >= 0:
            decimals = chartDict[IBCSdecimalName]
        return source_prefix, chartDict, decimals
