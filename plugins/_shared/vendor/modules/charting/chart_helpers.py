                ...
                try:
                    ...
                    if showTop:
                        ...
                        if chosenDimension not in cols:
                            return chartDict
                        dfDiscard, uniqueItems, aggregateOtherItemsName, valueCols = (
                            show_only_largest(
                                ensure_polars_df(df),
                                chosenDimension,
                                None,
...
def show_total_percent(fig, df, dfFiltered, period, metricToPlot, chartDict):
    ...
    fig.add_annotation(
        ...
        xref="x",
        yref="paper",
        # Plotly 6 no longer accepts paper-relative arrow references here.
        # A vertical pixel offset preserves the legacy 100% marker while
        # keeping the arrow tip anchored to the Mekko 100% data edge.
        axref="pixel",
        ayref="pixel",
        align="center",
        yshift=0,
...
def get_filter_text_or_company_name(chartDict, paramDict):
    ...
    def compact_filter_values(values):
        if values is None or values is False or values == []:
            return ""
        ...
        return ", ".join(
            str(value).strip() for value in list(values)[:3] if str(value).strip()
        )
        ...
        for element in chartDict[filterDictName]:
            if toIncludeItems in chartDict[filterDictName][element]:
                if len(chartDict[filterDictName][element][toIncludeItems]) > 0:
                    ...
                    compact_values = compact_filter_values(includeFilterItem)
                    if compact_values and len(filter_parts) < 4:
                        filter_parts.append(f"{element} = {compact_values}")
                    includeFilterItem = (
            ...
            if toExcludeItems in chartDict[filterDictName][element]:
                if len(chartDict[filterDictName][element][toExcludeItems]) > 0:
                    ...
                    compact_values = compact_filter_values(excludeFilterItem)
                    if compact_values and len(filter_parts) < 4:
                        filter_parts.append(f"{element} != {compact_values}")
                    excludeFilterItem = (
    ...
    if isFiltered:
        if filter_parts:
            filterString = "; ".join(filter_parts[:4])
        else:
            separator = ""
    ...
    if isFiltered or isNumberFiltered:
        if companyNameKey in chartDict and chartDict[companyNameKey]:
            companyName = str(chartDict[companyNameKey]).strip()
            companyName = f"{companyName} | {filterString}".strip() + " "
        else:
            companyName = filterString + " "
    elif companyNameKey in chartDict and chartDict[companyNameKey]:
        companyName = str(chartDict[companyNameKey]) + " "
...
def set_up_tab_for_show_or_download_chart(
    ...
    with showChartTab:
        ui.plotly_chart(fig, width="content", config=configPlotlyDict, theme=None)
