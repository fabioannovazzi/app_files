#!/usr/bin/env python3
"""Repeatable local teaching sessions sharing the product's confirmed onboarding profile.
...
class TeachingStore(Store):
    ...
    def _profile(self) -> dict[str, Any]:
        return Store(self.root, plugin_root=self.plugin_root).status()
    ...
    def _validate_session(
        ...
        if (
            ...
            or state.get("product") != self.product
            ...
            or state.get("workflow_id") not in eligible_workflows(self.plugin_root)
        ):
            raise OnboardingError(
