#!/usr/bin/env python3
"""Persist the product's one-off OpenAI desktop onboarding using only the local stdlib.
...
# It is a transport exclusion, not a shared Vera profile.
...
def default_root(product: str) -> Path:
    """Use one OS-user location, independent of host, plugin version and cwd."""
    if sys.platform == "win32":
        base = Path(os.environ.get("LOCALAPPDATA", Path.home() / "AppData/Local"))
        return base / product.title() / "onboarding"
    return Path.home() / ".local" / "share" / product / "onboarding"
...
class Store:
    ...
    def __init__(self, root: Path | None = None, *, plugin_root: Path) -> None:
        self.plugin_root = plugin_root.resolve()
        manifest = _read(self.plugin_root / ".codex-plugin/plugin.json")
        self.product = manifest.get("name")
        if self.product not in {"clara", "lucia"}:
            raise OnboardingError("Select an installed Clara or Lucia plugin")
        self.root = (root or default_root(self.product)).expanduser().absolute()
    ...
    def status(self) -> dict[str, Any]:
        ...
        _validate(state)
        if (
            state.get("product") != self.product
            or enrollment.get("product") != self.product
        ):
            raise OnboardingError(
                "Profile belongs to another product; reconnect its own local directory"
            )
    ...
    def begin(self) -> dict[str, Any]:
        """Enroll once, including an existing product user without this new record."""
        status = self.status()
        ...
        with self._lock():
            ...
            _write(
                self.enrollment,
                {"onboarding_id": state["onboarding_id"], "product": self.product},
            )
            (self.root / MARKER).write_text(
                f"{self.product.title()} onboarding: local-only tutorial artifacts.\n"
            )
            (self.root / ".mparanza-teaching-local-only").write_text(
                "Local tutorial; no transmission.\n", encoding="utf-8"
            )
    ...
    def change(
        ...
        with self._lock():
            ...
            if state["phase"] == "complete" and action not in {"profile", "feedback"}:
                raise OnboardingError(
                    "Onboarding is already complete; use normal product workflows"
                )
            if action == "notes":
    ...
    def recover(self, source: Path) -> dict[str, Any]:
        ...
        _validate(restored)
        if restored.get("product") != self.product:
            raise OnboardingError("Recovery copy belongs to another product")
        with self._lock():
            enrollment = _read(self.enrollment)
            if (
                restored["onboarding_id"] != enrollment.get("onboarding_id")
                or enrollment.get("product") != self.product
            ):
                raise OnboardingError(
