...
def calculate_difference_in_percent(
    ...
    df = df.with_columns(pl.lit(None, dtype=pl.Float64).alias(differenceInPercent))
...
def prepare_data_for_multitier_column_plot(
    ...
    expected_periods = [acName, pyName]
    if filterDates in chartDict and chartDict[filterDates]:
        expected_periods = [acName, plName]
        if fcName in cols_after_pivot:
            expected_periods.append(fcName)
    missing_periods = [
        period for period in expected_periods if period not in cols_after_pivot
    ]
    if missing_periods:
        df = df.with_columns([pl.lit(0.0).alias(period) for period in missing_periods])
        cols_after_pivot, _ = get_schema_and_column_names(df)
    yArray: list[str] = []
    if filterDates in chartDict and chartDict[filterDates]:
        if fcName in cols_after_pivot:
