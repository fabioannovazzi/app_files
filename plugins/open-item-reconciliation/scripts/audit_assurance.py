"""Replayable mechanical assurance controls for Open-item Reconciliation.
...
ASSURANCE_SCHEMA_VERSION = "open_item_reconciliation.assurance.v2"
SOURCE_DECISION_TYPE = "open_item_reconciliation_source_mapping"
...
RUN_CACHE_DIRECTORY = ".open_item_reconciliation_cache"
PERIMETER_FIELDS = {
    "entity_ref",
    "party_ref",
    "currency",
...
def _current_client_engagement(
    ...
    if context_path is None:
        raise AssuranceRunError(
            "current Open-item Reconciliation customer-run context is unavailable"
        )
    try:
        ...
        raise AssuranceRunError(
            f"current Open-item Reconciliation customer-run context is invalid: {exc}"
        ) from exc
...
def validate_final_output_inventory(
    ...
    if payload["schema_version"] != "open_item_reconciliation.final_outputs.v1":
...
def _publish_final_outputs(
    ...
    content = {
        "schema_version": "open_item_reconciliation.final_outputs.v1",
...
def _workflow_output_contract(
    ...
    content = {
        "schema_version": "open_item_reconciliation.output_contract.v1",
...
def _validate_workflow_output_contract(value: object) -> dict[str, Any]:
    ...
    if value["schema_version"] != "open_item_reconciliation.output_contract.v1":
    ...
    *,
    ...
    if entries:
        numeric_ledger = build_numeric_evidence_ledger(
            entries,
            ledger_id="open_item_reconciliation_material_values",
        )
...
def validate_assurance_run(
    ...
    *,
    ...
    if (
        ...
        or prepared["schema_version"] != "open_item_reconciliation.prepared_records.v2"
...
def _command_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Replay Open-item Reconciliation assurance controls."
    )
    parser.add_argument(
        "--client-engagement",
        type=Path,
