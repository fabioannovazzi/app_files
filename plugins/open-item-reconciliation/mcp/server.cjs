...
const IMPLEMENTATION_CONTRACT = [
  ["plugin", "assets/open-item-reconciliation-review-widget.html"],
...
const SERVER_NAME = "open-item-reconciliation-widgets";
...
const WIDGET_URI = "ui://widget/open-item-reconciliation-review.html";
const WIDGET_MIME_TYPE = "text/html;profile=mcp-app";
const MAX_ITEMS = 2500;
const MAX_PAYLOAD_BYTES = 2_000_000;
const TOOL_NAMES = {
  validateReview: "validate_open_item_reconciliation_review",
  renderReview: "render_open_item_reconciliation_review",
  caseContext: "get_open_item_reconciliation_case_context",
  saveDecisions: "save_open_item_reconciliation_decisions",
  applyDecisions: "apply_open_item_reconciliation_decisions",
};
...
const TOOL_INVOCATION_LABELS = {
  [TOOL_NAMES.renderReview]: {
    invoking: "Rendering Open-item Reconciliation review",
    invoked: "Rendered Open-item Reconciliation review",
  },
};
const WIDGET_DESCRIPTION =
  "Interactive Open-item Reconciliation review surface for deterministic row checks, exceptions, missing evidence, workpaper artifacts, and follow-up requests.";
const SERVER_INSTRUCTIONS = [
  "For a normal Open-item Reconciliation run, the primary visible handoff is the",
  ...
  "the reviewer.",
  "This MCP server remains an optional integrated Codex review surface. When",
  "using it, call validate_open_item_reconciliation_review with the complete review",
  ...
  "render_open_item_reconciliation_review. Use",
  "get_open_item_reconciliation_case_context for at most 25 specifically selected",
  "cases at a time; request exact identifiers only when the accounting judgment",
  "requires them. The render tool returns",
  "openai/outputTemplate for ui://widget/open-item-reconciliation-review.html.",
  "Use save_open_item_reconciliation_decisions to persist",
  "reviewer actions to ui_decisions.json and apply_open_item_reconciliation_decisions",
...
const SERVER_INSTRUCTIONS_ES = [
  "En una ejecución normal de Open-item Reconciliation, la entrega visible principal es el",
  ...
  "Este servidor MCP sigue siendo una superficie integrada opcional de revisión en Codex.",
  "Cuando lo use, llame a validate_open_item_reconciliation_review con el payload completo de revisión",
  "o proporcione rutas locales de la ejecución, como review_payload_path y run_intake_path, antes de",
  "render_open_item_reconciliation_review. Use save_open_item_reconciliation_decisions para guardar las",
  "acciones del revisor en ui_decisions.json y apply_open_item_reconciliation_decisions para escribir",
  ...
  return [
    {
      ...
      title: "Validate Open-item Reconciliation review payload",
      description:
        "Validate the Open-item Reconciliation review-session payload before optional MCP rendering. Call this first, then render_open_item_reconciliation_review when using the integrated Codex surface.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Open-item Reconciliation review",
      description:
        "Render an Open-item Reconciliation review-session payload as an optional MCP app for row review, checks, evidence requests, and artifacts. The normal visible handoff is the local browser server from scripts/review_server.py; call validate_open_item_reconciliation_review first when using this integrated Codex surface.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Get selected Open-item Reconciliation case context",
      description:
    ...
    },
    {
      ...
      title: "Save Open-item Reconciliation review decisions",
      description:
        "Validate Open-item Reconciliation review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Open-item Reconciliation review decisions",
      description:
        "Validate Open-item Reconciliation review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: true,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "open_item_reconciliation_review_widget",
      title: "Open-item Reconciliation review widget",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
  throw new Error(`unknown Open-item Reconciliation widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "open-item-reconciliation-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "open-item-reconciliation") {
    throw new Error('review_payload.plugin must be "open-item-reconciliation"');
  }
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(
      `Open-item Reconciliation widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`,
    );
  }
  return payload;
}
...
function auditReviewPythonCandidates() {
  const candidates = [
    process.env.OPEN_ITEM_RECONCILIATION_PYTHON,
...
function auditReviewRequireMatchingCustomerRun(inputArgs, outputDir) {
  ...
  if (
    ...
    throw new Error(
      "Open-item Reconciliation review run does not match the customer-run context.",
    );
  }
  return boundary;
}
...
function auditReviewProfessionalAuthority(
  ...
  const content = {
    schema_version: "open_item_reconciliation.professional_review.v1",
...
function initializeAuditReviewTransactionState(state, trustedImage, inputArgs) {
  ...
  if (
    ...
    throw new Error(
      "Persisted run intake, review payload, and final artifacts are required before Open-item Reconciliation review writes.",
    );
  }
  if (
    ...
    throw new Error(
      "Caller run intake does not match the persisted Open-item Reconciliation run intake.",
    );
  }
  if (
    ...
    throw new Error(
      "Caller review payload does not match the persisted Open-item Reconciliation review payload.",
    );
  }
  if (
    ...
    throw new Error(
      "Caller final artifacts do not match the persisted Open-item Reconciliation final artifacts.",
    );
  }
  if (
    ...
    throw new Error(
      "Caller UI decisions do not match the persisted Open-item Reconciliation UI decisions.",
    );
  }
...
function validateAuditParentTransactionState(
  ...
  if (!state?.complete) {
    throw new Error("Open-item Reconciliation parent transaction state is incomplete.");
  }
  ...
  if (
    ...
    throw new Error("Open-item Reconciliation write authorization did not close.");
  }
  if (
    JSON.stringify(persistedUiDecisions) !==
    JSON.stringify(state.expectedUiDecisions)
  ) {
    throw new Error("Open-item Reconciliation UI receipt did not close.");
  }
  if (kind === "apply") {
    if (
      ...
      throw new Error("Open-item Reconciliation parent application did not close.");
    }
    if (state.expectedRunIntake != null) {
      ...
      if (
        ...
        throw new Error("Open-item Reconciliation run receipt did not close.");
      }
    }
    if (state.expectedProfessionalReview != null) {
      ...
      if (
        ...
        throw new Error(
          "Open-item Reconciliation professional review authority did not close.",
        );
      }
    }
    for (const relativePath of state.expectedTransitionHistoryPaths || []) {
      ...
      if (
        ...
        throw new Error(
          "Open-item Reconciliation review transition did not close.",
        );
      }
    }
    if (state.expectedReviewHandoffContent != null) {
      ...
      if (
        ...
        throw new Error("Open-item Reconciliation review handoff did not close.");
      }
    }
  }
}
...
function validateAuditReconciliationReviewTransaction(
  ...
  if (!isPlainObject(result) || result.ok !== true || result.persisted !== true) {
    throw new Error("Open-item Reconciliation review transaction result is invalid.");
  }
  const requiredPaths =
    kind === "save"
      ? ["ui_decisions.json"]
  ...
  if (!requiredPaths.every((relativePath) => filePaths.has(relativePath))) {
    throw new Error("Open-item Reconciliation review transaction is incomplete.");
  }
  ...
  if (kind === "save") {
    ...
    const expectedResult = {
      ok: true,
      validation_type: "open_item_reconciliation_decisions",
      ...
      message: isSpanishRuntime(inputArgs)
        ? `Se han guardado ${persistedUiDecisions?.decision_count} decisiones de Open-item Reconciliation.`
        : `Saved ${persistedUiDecisions?.decision_count} Open-item Reconciliation decisions.`,
      ui_decisions: persistedUiDecisions,
    };
    if (!reviewResponseMatches(result, expectedResult)) {
      throw new Error("Open-item Reconciliation saved decisions did not close.");
    }
  } else {
    ...
    if (
      ...
      throw new Error(
        "Open-item Reconciliation review transaction is incomplete.",
      );
    }
    validateAuditParentTransactionState(
      kind,
    ...
    if (
      ...
      throw new Error("Open-item Reconciliation applied decisions did not close.");
    }
    if (
      ...
      throw new Error(
        "Open-item Reconciliation review decision state did not close.",
      );
    }
    const expectedResult = {
      ok: true,
      validation_type: "open_item_reconciliation_application",
      ...
      message: isSpanishRuntime(inputArgs)
        ? `Se han aplicado ${persistedAppliedDecisions.decision_count} decisiones de Open-item Reconciliation.`
        : `Applied ${persistedAppliedDecisions.decision_count} Open-item Reconciliation decisions.`,
      applied_decisions: persistedAppliedDecisions,
      final_artifacts: persistedFinalArtifacts,
    };
    if (!reviewResponseMatches(result, expectedResult)) {
      throw new Error("Open-item Reconciliation response did not close.");
    }
  }
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return withGeneratedReviewOutputTransaction(
    ...
    },
    {
      ...workflowOptions,
      failureMessage:
        "Open-item Reconciliation review save transaction failed safely.",
      rollbackFailureMessage:
        "Open-item Reconciliation review save transaction could not be restored safely.",
    },
  );
}
...
function saveDecisionPayloadWrites(inputArgs) {
  ...
  const result = {
    ...
    message: isSpanishRuntime(materializedArgs)
      ? persisted
        ? `Se han guardado ${uiDecisions.decision_count} decisiones de Open-item Reconciliation.`
        : "Las decisiones se han validado. No se proporcionó run_intake.output_dir, por lo que no se escribió ningún archivo."
      : persisted
        ? `Saved ${uiDecisions.decision_count} Open-item Reconciliation decisions.`
...
function resolveRunOutputDir(inputArgs) {
  ...
  if (!contextValue || !path.isAbsolute(contextValue)) {
    throw new Error("Open-item Reconciliation persistence requires the current client_engagement context.");
  }
  const contextPath = path.resolve(contextValue);
  if (contextPath !== contextValue || path.basename(contextPath) !== "context.json") {
    throw new Error("Open-item Reconciliation client_engagement path is invalid.");
  }
  ...
  if (
    ...
    throw new Error("Open-item Reconciliation client_engagement context is unavailable.");
  }
  if (!path.isAbsolute(outputReference) && runIntake?.path_reference !== "run_root_relative") {
    throw new Error("Open-item Reconciliation output reference is not run-root-relative.");
  }
  ...
  if (
    ...
    throw new Error("Open-item Reconciliation output reference leaves the customer run.");
  }
  return resolved;
}
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "open_item_reconciliation"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function writeDirectTextArtifactUpdates(outputDir, effects) {
  ...
  for (const effect of effects) {
    ...
    if (stat.isSymbolicLink() || !stat.isFile() || stat.nlink !== 1) {
      throw new Error("Open-item Reconciliation review target is unsafe.");
    }
...
function writeStructuredArtifactUpdates(outputDir, effects) {
  ...
  for (const effect of effects) {
    ...
    if (!stat) continue;
    if (stat.isSymbolicLink() || !stat.isFile() || stat.nlink !== 1) {
      throw new Error("Open-item Reconciliation structured review target is unsafe.");
    }
    const backupRelativePath = originalBackupRelativePath(effect, target.relativePath);
    const backupAbsolutePath = path.join(outputDir, backupRelativePath);
    fs.mkdirSync(path.dirname(backupAbsolutePath), { recursive: true });
...
function ensureReviewHandoffCard(inputArgs, outputDir) {
  ...
  const localizedAuditHandoff =
    pluginName === "open-item-reconciliation" && isSpanishRuntime(inputArgs);
...
function applyDecisionPayload(inputArgs) {
  ...
  return withGeneratedReviewOutputTransaction(
    ...
    },
    {
      ...workflowOptions,
      failureMessage:
        "Open-item Reconciliation review apply transaction failed safely.",
      rollbackFailureMessage:
        "Open-item Reconciliation review apply transaction could not be restored safely.",
    },
  );
}
...
function applyDecisionPayloadWrites(inputArgs) {
  ...
  const result = {
    ...
    message: isSpanishRuntime(materializedArgs)
      ? persisted
        ? `Se han aplicado ${responseAppliedDecisions.decision_count} decisiones de Open-item Reconciliation.`
      ...
      : persisted
        ? `Applied ${responseAppliedDecisions.decision_count} Open-item Reconciliation decisions.`
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    result.validation_type = "open_item_reconciliation_review";
    ...
    result.message = isSpanishRuntime(issued.context.privatePayload)
      ...
      : "Open-item Reconciliation review payload is valid. The complete payload stays out of model context; use the opaque reference to render the widget and request only cases that need interpretation.";
    return result;
  }
  ...
  throw new Error(`unknown Open-item Reconciliation widget tool: ${name}`);
}

function widgetUriForPayload(payload) {
  if (payload.widget_type === "open_item_reconciliation_review") return WIDGET_URI;
  return null;
}
