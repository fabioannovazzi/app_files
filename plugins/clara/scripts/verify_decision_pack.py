"""Verify current decision-pack identity before final deliverable review."""
...
from advisor_case_core import CaseWorkspaceError, verify_decision_pack
...
def main() -> int:
    """Check case and output bytes without certifying semantic correctness."""
    ...
    try:
        receipt = verify_decision_pack(args.case_dir, output_dir=args.output_dir)
    except (CaseWorkspaceError, OSError, ValueError) as exc:
        LOGGER.error("Decision pack verification failed: %s", exc)
        return 1
