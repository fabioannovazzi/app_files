...
material, but do not claim that a durable local file or deterministic validation
report exists. In Codex or Cowork, use the connected files and local helper to
write the durable artifacts. Do not make local tooling a prerequisite for a
useful in-chat assignment plan.
