...
The complete renderer requires Codex or another local Clara runtime with Python,
FFmpeg, and an `OPENAI_API_KEY` supplied through the local environment. In
ChatGPT, Clara may still prepare and review the scene plan and narration, but it
...
- OpenAI speech request shape, scene timing, caption timing, motion rendering,
...
validation status, and any residual issue. Write `codex_run_review.md` when the
...
`narration_script.md`, and `review_packet.md`. It does not call OpenAI speech or
render media.
...
images or source-basis notes—will be sent to the OpenAI speech endpoint using
...
If network access is blocked, rerun this exact render command with Codex host
...
The renderer uses the established English video voice policy: OpenAI
