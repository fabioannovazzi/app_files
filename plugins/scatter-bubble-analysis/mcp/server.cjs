...
const SERVER_NAME = "scatter-bubble-widgets";
...
const WIDGET_URI = "ui://widget/scatter-bubble-review.html";
const WIDGET_MIME_TYPE = "text/html;profile=mcp-app";
const MAX_ITEMS = 2500;
const MAX_PAYLOAD_BYTES = 2_000_000;
const TOOL_NAMES = {
  validateReview: "validate_scatter_bubble_review",
  renderReview: "render_scatter_bubble_review",
  saveDecisions: "save_scatter_bubble_decisions",
  applyDecisions: "apply_scatter_bubble_decisions",
};
...
function toolUiMeta(resourceUri, toolName = null) {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] = "Rendering Scatter Bubble review";
    meta["openai/toolInvocation/invoked"] = "Rendered Scatter Bubble review";
  }
  return meta;
}
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Scatter Bubble review surface for relationship drivers, generated charts, source contexts, follow-up requests, and final artifacts.",
  ...
  return [
    {
      ...
      title: "Validate Scatter Bubble review payload",
      description:
        "Validate the Scatter Bubble review-session payload before rendering. Call this first, then render_scatter_bubble_review.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Scatter Bubble review",
      description:
        "Render a Scatter Bubble review-session payload as an MCP HTML widget for relationship drivers, charts, source contexts, follow-ups, and artifacts.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Scatter Bubble review decisions",
      description:
        "Validate Scatter Bubble review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Scatter Bubble review decisions",
      description:
        "Validate Scatter Bubble review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "scatter_bubble_review_widget",
      title: "Scatter Bubble review widget",
      description:
        "Renders Scatter Bubble review-session payloads with searchable relationship drivers, chart artifacts, contexts, and follow-up requests.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
    throw new Error(`unknown Scatter Bubble widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "scatter-bubble-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "scatter-bubble-analysis") {
    throw new Error('review_payload.plugin must be "scatter-bubble-analysis"');
  }
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`Scatter Bubble widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "scatter_bubble_decisions",
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "scatter_bubble"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "scatter_bubble_application",
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    return {
      ok: true,
      validation_type: "scatter_bubble_review",
  ...
  throw new Error(`unknown Scatter Bubble widget tool: ${name}`);
}
