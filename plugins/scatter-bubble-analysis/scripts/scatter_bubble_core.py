...
def build_summary_markdown(
    ...
    mappings = recipe["mappings"]
    lines = [
        "# Scatter & Bubble Source Data",
        "",
        f"- Source file: `{recipe['source_file']}`",
        f"- X metric: `{mappings['x_metric_column']}`",
        f"- Y metric: `{mappings['y_metric_column']}`",
        f"- Bubble size metric: `{mappings['bubble_size_metric_column']}`",
        f"- Dot dimension: `{mappings['dot_dimension']}`",
    ...
    lines.extend(
        [
            "",
            "Charts are generated by the vendored legacy charting pipeline. Failed attempts are listed in the audit rather than replaced by non-legacy redraws.",
            "",
        ]
    )
    return "\n".join(lines)
...
def write_client_report(
    ...
    lines = [
        ...
        "",
        f"Charts compare `{recipe['mappings']['x_metric_column']}` to `{recipe['mappings']['y_metric_column']}`.",
        "",
        "## Largest Dots",
        "",
    ]
    ...
    try:
        ...
        document = Document()
        document.add_heading("Scatter & Bubble Analysis", level=1)
        document.add_paragraph(
            f"Charts compare {recipe['mappings']['x_metric_column']} to "
            f"{recipe['mappings']['y_metric_column']}."
        )
        document.add_heading("Largest Dots", level=2)
        for row in summary_table.head(5).to_dicts():
            document.add_paragraph(
                f"{row[dot_dimension]}: {_format_value(row[size_metric])} {size_metric}",
                style="List Bullet",
