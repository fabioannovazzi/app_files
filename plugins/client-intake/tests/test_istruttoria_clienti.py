...
from build_intake_outputs import build_intake_outputs
...
def _call_mcp_server(messages: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    if node is None:
        pytest.skip("Node.js is required to exercise the Client Intake MCP server.")
...
def test_build_intake_outputs_extracts_geneva_zurich_and_uk_fields(
    tmp_path: Path,
) -> None:
    ...
    result = build_intake_outputs(customer, target_year=2025)
...
def test_build_intake_outputs_writes_expected_files(tmp_path: Path) -> None:
    ...
    assert run_intake["plugin"] == "client-intake"
    assert run_intake["workflow"] == "fascicolo-intake"
    ...
    assert review_payload["review_type"] == "client_intake_folder_review"
    ...
    assert "render_client_intake_review" in handoff_text
    assert "apply_client_intake_decisions" in handoff_text
    index_output = next(
        item
        for item in final_artifacts["outputs"]
...
def test_client_intake_mcp_server_validates_and_renders_review_payload() -> None:
    review_payload = {
        "schema_version": "1.0",
        "plugin": "client-intake",
        "workflow": "fascicolo-intake",
        "run_id": "client-intake-test-run",
    ...
    messages: list[dict[str, object]] = [
        {"jsonrpc": "2.0", "id": 1, "method": "tools/list"},
        {
            ...
            "method": "tools/call",
            "params": {
                "name": "validate_client_intake_review",
        ...
        },
        {
            ...
            "method": "tools/call",
            "params": {
                "name": "render_client_intake_review",
                "arguments": {"review_payload": review_payload},
            },
        },
        {"jsonrpc": "2.0", "id": 4, "method": "resources/list"},
        {
            ...
            "params": {"uri": "ui://widget/client-intake-review.html"},
        },
    ]
    ...
    assert render_result["structuredContent"]["widget_type"] == "client_intake_review"
    assert (
    ...
    assert "ui://widget/client-intake-review.html" in resource_uris
    widget_html = responses[5]["result"]["contents"][0]["text"]
    assert "Client Intake Review" in widget_html


def test_client_intake_mcp_apply_updates_draft_email_artifact(
    tmp_path: Path,
) -> None:
    ...
    run_intake = {
        ...
        "plugin": "client-intake",
        "workflow": "fascicolo-intake",
        "run_id": "client-intake-apply-test-run",
    ...
    messages: list[dict[str, object]] = [
        {
            ...
            "method": "tools/call",
            "params": {
                "name": "apply_client_intake_decisions",
                "arguments": {
