---
name: client-intake
description: "Use when a studio asks Codex to prepare the first intake of a customer folder for Italy, Geneva, Zurich, or the UK: run local checks, classify documents, read generated evidence, and write a concise operational pack."
---
...
This is the main Codex workflow. Use it when the user asks for an istruttoria clienti, customer-folder intake, fascicolo cliente review, preliminary 730/Redditi package review, Swiss Geneva/Zurich tax-folder intake, UK Self Assessment folder intake, or a first pass over a customer folder for an accounting studio.
...
```text
Prepare the first intake of a customer tax folder for Italy, Geneva, Zurich,
or the UK into inventory, formal checks, structured fields, a studio memo, and
a reviewable client email draft.
```

For Geneva, Zurich, and UK folders, present the same product as document
...
- output folder, if not a sibling `output/client-intake` folder next to `<cartella-cliente>`;
...
7. Read `run_intake.json` and `review_payload.json`. Treat the review payload
   ...
   call `save_client_intake_decisions` so `ui_decisions.json` is validated and
   persisted, then call `apply_client_intake_decisions` so
   `applied_decisions.json` and `final_artifacts.json` reflect the reviewed
   actions before revising final outputs.
8. When the `clientIntakeWidgets` MCP tools are available, use the OpenAI-style
   handoff: call `validate_client_intake_review` with the full review payload,
   then call `render_client_intake_review` once validation succeeds. When
   decisions are collected, call `save_client_intake_decisions` and
   `apply_client_intake_decisions` before treating the review as applied. The
...
`save_client_intake_decisions` to persist `ui_decisions.json`, then
`apply_client_intake_decisions` to write `applied_decisions.json` and update
`final_artifacts.json`.
