...
const SERVER_NAME = "client-intake-widgets";
...
const WIDGET_URI = "ui://widget/client-intake-review.html";
const WIDGET_MIME_TYPE = "text/html;profile=mcp-app";
const MAX_ITEMS = 2500;
const MAX_PAYLOAD_BYTES = 1_500_000;
const TOOL_NAMES = {
  validateReview: "validate_client_intake_review",
  renderReview: "render_client_intake_review",
  saveDecisions: "save_client_intake_decisions",
  applyDecisions: "apply_client_intake_decisions",
};
...
function toolUiMeta(resourceUri, toolName = null) {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] = "Rendering client intake review";
    meta["openai/toolInvocation/invoked"] = "Rendered client intake review";
  }
  return meta;
}
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Client Intake review surface for document inventory, missing documents, fiscal fields, memo, and client email draft.",
  ...
  return [
    {
      ...
      title: "Validate Client Intake review payload",
      description:
        "Validate the Client Intake review-session payload before rendering. Call this first, then render_client_intake_review.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Client Intake review",
      description:
        "Render a Client Intake review-session payload as an MCP HTML widget for document inventory, exceptions, fields, memo, and email review.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Client Intake review decisions",
      description:
        "Validate Client Intake review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Client Intake review decisions",
      description:
        "Validate Client Intake review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: true,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "client_intake_review_widget",
      title: "Client Intake review widget",
      description:
        "Renders Client Intake review-session payloads with searchable review items and source previews.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) throw new Error(`unknown Client Intake widget resource: ${uri}`);
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "client-intake-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "client-intake") {
    throw new Error('review_payload.plugin must be "client-intake"');
  }
  ...
  const payload = {
    widget_type: "client_intake_review",
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`client intake widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "client_intake_decisions",
    ...
    message: persisted
      ? `Saved ${uiDecisions.decision_count} Client Intake decisions.`
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "client_intake"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "client_intake_application",
    ...
    message: persisted
      ? `Applied ${responseAppliedDecisions.decision_count} Client Intake decisions.`
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    return {
      ok: true,
      validation_type: "client_intake_review",
      ...
      message: "Client Intake review payload is valid. It is safe to call render_client_intake_review once.",
  ...
  throw new Error(`unknown Client Intake widget tool: ${name}`);
}
  ...
  try {
    if (method === "initialize") {
      return rpcResponse(messageId, {
        ...
        instructions:
          "Use validate_client_intake_review before render_client_intake_review. Prefer the MCP widget for Client Intake review handoff; use save_client_intake_decisions to persist reviewer actions to ui_decisions.json and apply_client_intake_decisions to write applied_decisions.json plus final_artifacts.json status when decisions are collected; fall back to Markdown/static review only when MCP is unavailable.",
      });
    }
