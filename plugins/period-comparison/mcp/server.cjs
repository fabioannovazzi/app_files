...
const SERVER_NAME = "period-comparison-widgets";
...
const WIDGET_URI = "ui://widget/period-comparison-review.html";
const WIDGET_MIME_TYPE = "text/html;profile=mcp-app";
const MAX_ITEMS = 2500;
const MAX_PAYLOAD_BYTES = 2_000_000;
const TOOL_NAMES = {
  validateReview: "validate_period_comparison_review",
  renderReview: "render_period_comparison_review",
  saveDecisions: "save_period_comparison_decisions",
  applyDecisions: "apply_period_comparison_decisions",
};
...
function responseCopy(inputArgs) {
  if (requestLanguage(inputArgs) !== "es") {
    return {
      saved: (count) => `Saved ${count} Period Comparison decisions.`,
      saveWithoutPath:
        "Validated decisions. No run_intake.output_dir was provided, so nothing was written.",
      applied: (count) => `Applied ${count} Period Comparison decisions.`,
      applyWithoutPath:
        "Validated applied decisions. No run_intake.output_dir was provided, so nothing was written.",
      valid:
        "Period Comparison review payload is valid. It is safe to call render_period_comparison_review once.",
      instructions:
        "Use validate_period_comparison_review before render_period_comparison_review. Prefer the MCP widget for Period Comparison review handoff; use save_period_comparison_decisions to persist reviewer actions to ui_decisions.json and apply_period_comparison_decisions to write applied_decisions.json plus final_artifacts.json status when decisions are collected; fall back to Markdown/static review only when MCP is unavailable.",
    };
  }
  return {
    saved: (count) => `Se han guardado ${count} decisiones de la revisión de comparación de periodos.`,
    saveWithoutPath:
      "Las decisiones son válidas. No se ha indicado run_intake.output_dir, por lo que no se ha escrito ningún archivo.",
    applied: (count) => `Se han aplicado ${count} decisiones de la revisión de comparación de periodos.`,
    applyWithoutPath:
      "Las decisiones aplicadas son válidas. No se ha indicado run_intake.output_dir, por lo que no se ha escrito ningún archivo.",
    valid:
      "Los datos de revisión de comparación de periodos son válidos. Ya puede llamar una vez a render_period_comparison_review.",
    instructions:
      "Use validate_period_comparison_review antes de render_period_comparison_review. Utilice preferentemente el widget MCP para entregar la revisión; use save_period_comparison_decisions para guardar las decisiones en ui_decisions.json y apply_period_comparison_decisions para escribir applied_decisions.json y actualizar el estado de final_artifacts.json. Recurra a la revisión Markdown o estática solo si MCP no está disponible.",
  };
}
...
function toolUiMeta(resourceUri, toolName = null, language = "en") {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] =
      language === "es" ? "Abriendo la revisión de comparación de periodos" : "Rendering Period Comparison review";
    meta["openai/toolInvocation/invoked"] =
      language === "es" ? "Revisión de comparación de periodos abierta" : "Rendered Period Comparison review";
  }
  return meta;
}
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Period Comparison review surface for period movements, windows, generated charts, source contexts, follow-up requests, and final artifacts.",
  ...
  return [
    {
      ...
      title: "Validate Period Comparison review payload",
      description:
        "Validate the Period Comparison review-session payload before rendering. Call this first, then render_period_comparison_review.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Period Comparison review",
      description:
        "Render a Period Comparison review-session payload as an MCP HTML widget for period movements, charts, source contexts, follow-ups, and artifacts.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Period Comparison review decisions",
      description:
        "Validate Period Comparison review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Period Comparison review decisions",
      description:
        "Validate Period Comparison review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "period_comparison_review_widget",
      title: "Period Comparison review widget",
      description:
        "Renders Period Comparison review-session payloads with searchable movement rows, chart artifacts, contexts, and follow-up requests.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
    throw new Error(`unknown Period Comparison widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "period-comparison-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "period-comparison") {
    throw new Error('review_payload.plugin must be "period-comparison"');
  }
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`Period Comparison widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "period_comparison_decisions",
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "period_comparison"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "period_comparison_application",
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    return {
      ok: true,
      validation_type: "period_comparison_review",
  ...
  throw new Error(`unknown Period Comparison widget tool: ${name}`);
}
