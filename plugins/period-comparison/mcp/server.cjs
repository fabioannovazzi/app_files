...
function toolUiMeta(resourceUri, toolName = null) {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] = "Rendering Period Comparison review";
    meta["openai/toolInvocation/invoked"] = "Rendered Period Comparison review";
  }
  return meta;
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ...
    message: persisted
      ? `Saved ${uiDecisions.decision_count} Period Comparison decisions.`
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ...
    message: persisted
      ? `Applied ${responseAppliedDecisions.decision_count} Period Comparison decisions.`
