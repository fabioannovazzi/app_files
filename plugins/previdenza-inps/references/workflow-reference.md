...
Prepare a documented case file for professional review. Vera may register an official portal export or capture one already-authenticated INPS browser tab read-only. The user remains responsible for access authority and permitted use; Vera cannot prove that legal position and does not manufacture an approval record. This is not an INPS API, autonomous login, or authority to operate the portal. Do not submit filings, activate delegations, sign an opinion, or automatically assign a subject to a contribution regime. Preserve contrary evidence and residual uncertainty.
...
When the user selects browser assistance, `capture_portal_snapshot.py` attaches through a loopback-only CDP endpoint to exactly one tab that the user has already opened and authenticated. Invoking the command with the exact INPS origin is the route choice; the script does not collect an actor, reason, scope, checkbox set, or approval ID. The user remains responsible for access authority, profile or delegation, and permitted use. The capture code may read only rendered body text and a page image. It must not navigate, click, fill, submit, download, inspect cookies or storage, save HTML, export browser state, or close the user's browser.
