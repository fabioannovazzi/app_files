...
Before semantic work, record a `processing_authorization`: whether the studio may process the third-party records, whether the current model/runtime is approved, its processor scope, a stable approving actor ID and role, timestamp, basis, and confirmation that personal data was minimized. The deterministic local inventory may run while this is unresolved; do not expose evidence content to a model/provider until it is confirmed.
...
These booleans record reviewer decisions. The validator must never infer them. Also record one `decision_log` entry for every gate, with the gate name, decision, stable pseudonymous actor ID, approving role, recording timestamp, and specific documentary or user-instruction basis. The model is not an approving authority.
...
When browser assistance is technically available, `capture_portal_snapshot.py` remains blocked until the professional records a specific terms, contract, or mandate basis permitting software-assisted capture for that service. Access/delegation authority and client-data processing authority are separate required gates. Only then may it attach through a loopback-only CDP endpoint to exactly one tab that the authorized human has already opened and authenticated. The human owns login, profile selection, delegation, and any entry of personal identifiers. The capture code may read only rendered body text and a page image. It must not navigate, click, fill, submit, download, inspect cookies or storage, save HTML, export browser state, or close the user's browser. Require a pseudonymous approving actor, timezone-aware approval time, exact approved INPS origin, bounded scope and purpose, and explicit confirmations for all three authority gates, own-credential use, read-only scope, and absence of visible credentials or one-time codes.
...
Keep evidence local by default in a dedicated access-restricted output directory. Local OCR and loopback browser capture do not upload page images to a provider. If an approved run downloads missing OCR model weights, record the approving ID and actual network use separately; only public model files are retrieved. Local scripts making no content network calls do not imply that later semantic model processing is local: record the actual approved processor/runtime and authority separately before semantic review. Do not place raw identifiers, raw portal URLs, credentials, session cookies, tokens, document quotes, or unsanitized document-request text in the MCP review payload. The MCP projection is limited to opaque IDs, enumerated states, bounded counts, and approved artifact names, and persisted review actions must match the exact stored review payload. A persisted ready review also requires a regular, identity-matching final manifest that is itself ready and binds that review. Before every persisted render, save, or apply action, the MCP server rechecks the package's acquisition binding against the current acquisition posture, exact file-inventory bytes, and canonical portal receipts; a mismatch stops before any review artifact is written. Package-owned execution-trace additions are deliberately outside that immutable projection. Use case IDs and subject IDs in review surfaces. Minimize duplicate evidence copies and exclude personal identifiers from external research queries. Retention, encryption, secure sharing, and deletion remain the studio's responsibility unless a separately approved storage policy controls them.
