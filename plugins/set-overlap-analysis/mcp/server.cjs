...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ...
    message: persisted
      ? `Saved ${uiDecisions.decision_count} Set Overlap decisions.`
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ...
    message: persisted
      ? `Applied ${responseAppliedDecisions.decision_count} Set Overlap decisions.`
...
function handleRpc(message) {
  ...
  try {
    ...
    if (method === "tools/call") {
      return rpcResponse(messageId, toolError(error.message));
    }
