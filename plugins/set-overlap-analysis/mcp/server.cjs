...
const SERVER_NAME = "set-overlap-widgets";
...
const WIDGET_URI = "ui://widget/set-overlap-review.html";
const WIDGET_MIME_TYPE = "text/html;profile=mcp-app";
const MAX_ITEMS = 2500;
const MAX_PAYLOAD_BYTES = 2_000_000;
const TOOL_NAMES = {
  validateReview: "validate_set_overlap_review",
  renderReview: "render_set_overlap_review",
  saveDecisions: "save_set_overlap_decisions",
  applyDecisions: "apply_set_overlap_decisions",
};
...
const RUNTIME_COPY = {
  en: {
    ...
    validationMessage:
      "Set Overlap review payload is valid. It is safe to call render_set_overlap_review once.",
    saved: (count) => `Saved ${count} Set Overlap decisions.`,
    saveNotWritten:
      "Validated decisions. No run_intake.output_dir was provided, so nothing was written.",
    applied: (count) => `Applied ${count} Set Overlap decisions.`,
    applyNotWritten:
  ...
  es: {
    ...
    validationMessage:
      "Los datos de revisión del solapamiento de conjuntos son válidos. Ya puede ejecutar render_set_overlap_review.",
    saved: (count) => `Se han guardado ${count} decisiones del solapamiento de conjuntos.`,
    saveNotWritten:
      "Las decisiones son válidas. No se ha proporcionado run_intake.output_dir, por lo que no se ha escrito ningún archivo.",
    applied: (count) => `Se han aplicado ${count} decisiones del solapamiento de conjuntos.`,
    applyNotWritten:
...
function toolUiMeta(resourceUri, toolName = null) {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] = "Rendering Set Overlap review";
    meta["openai/toolInvocation/invoked"] = "Rendered Set Overlap review";
  }
  return meta;
}
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Set Overlap review surface for selected sets, exact intersections, pairwise overlaps, chart audits, contexts, and generated artifacts.",
  ...
  return [
    {
      ...
      title: "Validate Set Overlap review payload",
      description:
        "Validate the Set Overlap Analysis review-session payload before rendering. Call this first, then render_set_overlap_review.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Set Overlap review",
      description:
        "Render a Set Overlap Analysis review-session payload as an MCP HTML widget for set summaries, exact intersections, pair overlaps, chart audits, contexts, and artifacts.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Set Overlap review decisions",
      description:
        "Validate Set Overlap review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Set Overlap review decisions",
      description:
        "Validate Set Overlap review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "set_overlap_review_widget",
      title: "Set Overlap review widget",
      description:
        "Renders Set Overlap Analysis review-session payloads with searchable set summaries, intersections, chart audits, contexts, and artifacts.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
    throw new Error(`unknown Set Overlap widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "set-overlap-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "set-overlap-analysis") {
    throw new Error('review_payload.plugin must be "set-overlap-analysis"');
  }
  ...
  const payload = {
    widget_type: "set_overlap_review",
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`Set Overlap widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "set_overlap_decisions",
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "set_overlap"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "set_overlap_application",
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    return {
      ok: true,
      validation_type: "set_overlap_review",
  ...
  throw new Error(`unknown Set Overlap widget tool: ${name}`);
}
...
function handleRpc(message) {
  ...
  try {
    ...
    if (method === "tools/call") {
      ...
      return rpcResponse(
        messageId,
        toolError(localizeErrorMessage(error.message, args)),
      );
    }
    return rpcError(messageId, -32603, error.message);
  }
}
