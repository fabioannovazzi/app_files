...
After substantive use, follow the invoking product router's `Plugin Improvement
Feedback` instructions. Keep technical improvement notes free of client data and
source details, local to chat or run artifacts.
...
or material steps. Local inspection, calculation, rendering, validation and
already authorized run-output writes do not add an approval ceremony.
...
action. Include `codex_run_review.md` when the host needs a review note. Build
