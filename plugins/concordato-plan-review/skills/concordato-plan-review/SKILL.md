...
`structuredContent`. The same component-only payload contract applies in Codex
and Cowork when the optional interface is callable and used. If it is
unavailable in either runtime, use the same file-based fallback: begin with the
delivered semantic review and open only the exact review or source files needed
for the professional question. The 25-item tool limit does not apply to that
fallback.
