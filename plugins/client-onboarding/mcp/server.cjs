...
const SERVER_NAME = "client-onboarding-review";
const PLUGIN_NAME = "client-onboarding";
...
const WIDGET_URI = "ui://widget/client-onboarding-review.html";
...
const TOOL_NAMES = {
  validate: "validate_client_onboarding_review",
  render: "render_client_onboarding_review",
  save: "save_client_onboarding_decisions",
  apply: "apply_client_onboarding_decisions",
};
...
function toolUiMeta() {
  return {
    ...
    "openai/toolInvocation/invoking": "Rendering client onboarding review",
    "openai/toolInvocation/invoked": "Rendered client onboarding review",
  };
}
  ...
  return [
    {
      ...
      title: "Validate client onboarding review",
    ...
    },
    {
      ...
      title: "Open client onboarding review",
      description:
    ...
    },
    {
      ...
      title: "Save client onboarding decisions",
      description:
    ...
    },
    {
      ...
      title: "Apply client onboarding decisions",
      description:
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "client_onboarding_review_widget",
      title: "Client onboarding review",
      description: "Interactive professional review of a privacy-minimized onboarding case.",
...
function resourceText(uri) {
  ...
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "client-onboarding-review-widget.html"),
    "utf8",
  );
}
...
function validateReview(input) {
  ...
  const payload = {
    widget_type: "client_onboarding_review",
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`client onboarding widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function verifyLocalArtifactBindings(outputDir, review, reviewHash, finalArtifacts) {
  ...
  if (
    ...
    || review.basis_hashes.intake !== facts.input_hash
  ) {
    throw new Error("review_payload.basis_hashes.intake does not match case_facts_validated.json");
  }
  const expectedBasis = {
...
function saveDecisions(input) {
  ...
  return {
    ok: true,
    validation_type: "client_onboarding_decisions",
    ...
    message: outputPath
      ? `Saved ${uiDecisions.decision_count} client onboarding decisions.`
...
function applyDecisions(input) {
  ...
  return {
    ok: true,
    validation_type: "client_onboarding_application",
    run_id: review.run_id,
    review_payload_sha256: reviewHash,
    decision_count: uiDecisions.decision_count,
    item_count: review.items.length,
...
function callTool(name, input) {
  if (name === TOOL_NAMES.validate) {
    ...
    return {
      ok: true,
      validation_type: "client_onboarding_review",
  ...
  if (name === TOOL_NAMES.render) {
    ...
    try {
      ...
      if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
        throw new Error(
          `client onboarding widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`,
        );
      }
      return payload;
    } catch (error) {
