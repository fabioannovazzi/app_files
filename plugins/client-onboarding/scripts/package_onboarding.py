...
from onboarding_core import (  # noqa: E402
    EXPECTED_ARTIFACTS,
    SCHEMA_VERSION,
    ValidationError,
...
__all__ = ["build_parser", "main", "package_onboarding"]
...
def _run_id(generated_at: str, input_hash: str) -> str:
    ...
    return f"client-onboarding-{timestamp}-{input_hash[:12]}"
...
def _write_client_missing_information_draft(
    ...
    *,
    ...
    lines = [
        "# Draft — request for missing onboarding information",
        "",
...
def _write_review_handoff(path: Path, *, run_id: str) -> Path:
    return write_private_text(
        ...
        "\n".join(
            [
                "# Client Onboarding Review Handoff",
                "",
                f"- Run ID: `{run_id}`",
                ...
                "",
                "Review tools: `validate_client_onboarding_review`, "
                "`render_client_onboarding_review`, "
                "`save_client_onboarding_decisions`, and "
...
def package_onboarding(
    ...
    *,
    ...
    """Build a complete, private and reviewable client-onboarding package."""

    generated_at_value = generated_at or utc_now()
    resolved_input = input_path.expanduser().resolve()
    intake = validate_intake(load_json(resolved_input))
    ...
    if existing:
        raise ValidationError(
            "Output directory already contains onboarding artifacts. Preserve that "
    ...
    client_intake_verification = verify_client_intake_binding(
        intake, base_dir=resolved_input.parent
    )
    ...
    if client_intake_verification.get("bound_manifest_path") is not None:
        local_files_read.append(client_intake_verification["bound_manifest_path"])
    local_files_read.extend(
    ...
    run_intake = {
        ...
        "inferred_task": "prepare_reviewable_professional_client_onboarding",
        "assumptions": [
        ...
        "execution_trace": [
            {
                ...
                "command": ["package_onboarding.py", "--input", "<local-input>"],
                "inputs": [
                    *local_files_read,
                ],
            ...
            },
            {
                ...
                "professional_review_required": True,
                "execution_location": "local_codex_workspace",
                "command": ["package_onboarding.py", "calculate_aml"],
            ...
            },
            {
                ...
                "professional_review_required": True,
                "execution_location": "local_codex_workspace",
                "command": ["package_onboarding.py", "build_review_artifacts"],
                "inputs": [
    ...
    written_paths = [
        ...
        _write_memo(
            resolved_output / "studio_onboarding_memo.md",
            intake=intake,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Build a private, reviewable Vera client-onboarding package."
    )
