...
`output/client-onboarding-<run-id>` folder next to the user's private source
folder. Pass that exact path to every `--output-dir` option.

# Client Onboarding

Prepare the professional relationship for review; do not pronounce the client
"onboarded" merely because files were generated. The strongest workflow state
...
`plugins/client-onboarding`.

## Product boundary

Use sibling `client-intake` for recursive inventory, OCR, extraction, and
document classification. When a reviewed Client Intake run exists, bind to its
...
as reviewed Client Intake evidence.

Client Onboarding owns:
...
2. Show a Run Intake table with the pseudonymous case alias, verified Client
   Intake binding or explicit standalone-evidence mode, output directory,
...
python scripts/initialize_case.py \
  --case-dir /private/path/client-onboarding-run \
...
Prefer a reviewed `client-intake` run. Verify the supplied run ID, exact
...
```bash
python scripts/package_onboarding.py \
  --input /private/path/client-onboarding-run/onboarding_intake.json \
  --output-dir /private/path/client-onboarding-run
```
...
template. Client Onboarding emits an internal document plan, not client-ready
legal wording.
...
1. call `validate_client_onboarding_review`;
2. call `render_client_onboarding_review`;
3. persist actions with `save_client_onboarding_decisions`;
4. apply them with `apply_client_onboarding_decisions`, producing
...
- `onboarding_intake.json` (private source intake, retained beside the package);
...
- `monitoring_plan.json` and `studio_onboarding_memo.md`;
