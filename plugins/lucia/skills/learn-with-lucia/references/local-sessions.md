...
Resolve `<lucia-root>` as the directory containing this skill's parent `skills`
...
```json
{"workflow_id": "apertura-pratica", "title": "Nuovo fascicolo", "goal": "Capire quali informazioni e verifiche servono", "mode": "show"}
```
...
```text
python3 <lucia-root>/scripts/local_teaching.py <command> --session <id> --revision <revision> --input <local-json>
```
...
```text
python3 <lucia-root>/scripts/local_teaching.py worker --session <id> --thread-id <actual-native-id> --workflow <id> --token <current-token>
```
...
```text
python3 <lucia-root>/scripts/local_onboarding_case.py --session <id> --thread-id <actual-worker-id> --workflow <workflow> --token <token> --phase demo --source <selected-source>
```
...
`../../lucia/references/tutorial-cases.md` apply. One adapter call creates a new
