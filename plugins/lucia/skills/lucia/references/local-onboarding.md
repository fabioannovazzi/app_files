# First conversation with Lucia

This is Lucia's mandatory, one-off onboarding for **Codex desktop**. It also
loads and resumes the same profile in **local ChatGPT Work on the same OS user
account**. An existing Lucia user gets this introduction once when no completed record
...
`../../learn-with-lucia/SKILL.md`. A working chat carrying a native teacher's
...
Before the first substantive Lucia action in every session, including direct
specialist invocation, resolve the installed Lucia root from this reference and
run `python3 <lucia-root>/scripts/local_onboarding.py status` with the host's
...
The default directory is `~/.local/share/lucia/onboarding` on macOS/Linux and
`%LOCALAPPDATA%/Lucia/onboarding` on Windows. It is independent of the current
...
- `required`: start in a native Codex teaching chat, run `begin`, then conduct
  ...
  that Codex chat. Do this for established Lucia users too; do not infer completion
  from earlier work.
...
Read `../../learn-with-lucia/SKILL.md` for native voice, the two parallel chats,
...
studio e che cosa ti piacerebbe provare con Lucia?”
...
and the first things they want to try. Ask whether they have already used Lucia;
...
`../../learn-with-lucia/SKILL.md`. Bind the native pair with `pair`, `start` the
...
`.lucia-onboarding-local-only` marker suppresses the receipt client's transmission
...
```text
python3 <lucia-root>/scripts/local_onboarding.py <command> --revision <latest-revision> --input <local-json>
```
...
in `../../learn-with-lucia/references/prepared-courses.md`. Read the current
...
`../../learn-with-lucia/references/execution-evidence.md`. Preserve the
