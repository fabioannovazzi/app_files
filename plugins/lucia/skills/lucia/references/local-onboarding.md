...
This is Lucia's optional, one-off introduction for **Codex desktop**, available
...
installed Lucia root and run
`python3 <lucia-root>/scripts/local_onboarding.py status` with the host's local execution tool. Use the shared managed Python
...
These statuses describe only the optional tutorial, never permission to use Lucia:
...
profile failures affect only the requested tutorial and never access to Lucia.
