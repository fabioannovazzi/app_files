#!/usr/bin/env python3
"""Check Lucia component availability and delegate dependency checks."""
...
def _component_root(name: str) -> Path:
    """Return the packaged or repository-source root for one component."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if missing:
        LOGGER.error("Missing Lucia components: %s", ", ".join(missing))
        return 1
    if args.module is None:
        LOGGER.info("All %s Lucia components are available.", len(COMPONENTS))
        return 0
