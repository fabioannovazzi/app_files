...
def add_numeric_totals_table(
    ...
    *,
    ...
    table.alignment = WD_TABLE_ALIGNMENT.LEFT
    table.style = "Table Grid"
    headers = ("Column", "Count", "Sum", "Range")
    for idx, header in enumerate(headers):
...
def write_report_docx(
    ...
    document.core_properties.comments = "Generated by the Build Report Codex plugin."
    ...
    for section_analysis in analysis.get("sections", []):
        ...
        if numeric_columns:
            ...
            add_numeric_totals_table(document, numeric_columns)
    ...
    add_key_value_table(
        document,
        (
            ("Input path", audit.get("input_path", "")),
            ("Tables discovered", audit.get("table_count", 0)),
            (
