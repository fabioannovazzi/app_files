...
const WIDGET_URI = "ui://widget/variance-analysis-review.html";
...
const TOOL_NAMES = {
  ...
  saveDecisions: "save_variance_analysis_decisions",
  applyDecisions: "apply_variance_analysis_decisions",
};
...
const RUNTIME_COPY = {
  en: {
    ...
    validationMessage:
      "Variance Analysis review payload is valid. It is safe to call render_variance_analysis_review once.",
    saved: (count) => `Saved ${count} Variance Analysis decisions.`,
    saveNotWritten:
      "Validated decisions. No run_intake.output_dir was provided, so nothing was written.",
    applied: (count) => `Applied ${count} Variance Analysis decisions.`,
    applyNotWritten:
  ...
  es: {
    ...
    validationMessage:
      "Los datos de revisión del análisis de variaciones son válidos. Ya puede ejecutar render_variance_analysis_review.",
    saved: (count) => `Se han guardado ${count} decisiones del análisis de variaciones.`,
    saveNotWritten:
      "Las decisiones son válidas. No se ha proporcionado run_intake.output_dir, por lo que no se ha escrito ningún archivo.",
    applied: (count) => `Se han aplicado ${count} decisiones del análisis de variaciones.`,
    applyNotWritten:
...
function toolUiMeta(resourceUri, toolName = null) {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] = "Rendering Variance Analysis review";
    meta["openai/toolInvocation/invoked"] = "Rendered Variance Analysis review";
  }
  return meta;
}
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Variance Analysis review surface for variance drivers, generated charts, source contexts, follow-up requests, and final artifacts.",
  ...
  return [
    {
      ...
      title: "Validate Variance Analysis review payload",
      description:
        "Validate the Variance Analysis review-session payload before rendering. Call this first, then render_variance_analysis_review.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Variance Analysis review",
      description:
        "Render a Variance Analysis review-session payload as an MCP HTML widget for drivers, charts, source contexts, follow-ups, and artifacts.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Variance Analysis review decisions",
      description:
        "Validate Variance Analysis review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Variance Analysis review decisions",
      description:
        "Validate Variance Analysis review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "variance_analysis_review_widget",
      title: "Variance Analysis review widget",
      description:
        "Renders Variance Analysis review-session payloads with searchable driver rows, chart artifacts, contexts, and follow-up requests.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
    throw new Error(`unknown Variance Analysis widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "variance-analysis-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "variance-analysis") {
    throw new Error('review_payload.plugin must be "variance-analysis"');
  }
  ...
  const payload = {
    widget_type: "variance_analysis_review",
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`Variance Analysis widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "variance_analysis_decisions",
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "variance_analysis"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "variance_analysis_application",
  ...
  try {
    if (method === "initialize") {
      return rpcResponse(messageId, {
        ...
        instructions:
          "Use validate_variance_analysis_review before render_variance_analysis_review. Prefer the MCP widget for Variance Analysis review handoff; use save_variance_analysis_decisions to persist reviewer actions to ui_decisions.json and apply_variance_analysis_decisions to write applied_decisions.json plus final_artifacts.json status when decisions are collected; fall back to Markdown/static review only when MCP is unavailable.",
      });
    }
