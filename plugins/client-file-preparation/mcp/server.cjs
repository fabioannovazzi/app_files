...
function preflightClientWorkflowRun(outputDir, expectedRunId) {
  ...
  if (completed.error || completed.status !== 0) {
    throw new Error(
      "Client File Preparation persistence requires a running v2 customer-folder workflow run",
    );
  }
  let result;
  try {
  ...
  if (
    ...
    || result.workflow_id !== "client-file-preparation"
...
function saveDecisionPayload(inputArgs) {
  ...
  if (persistence && decisionOutputPath) {
    preflightClientWorkflowRun(persistence.outputDir, reviewPayload.run_id);
...
function resolveRunOutputDir(inputArgs) {
  ...
  if (path.isAbsolute(outputDir)) {
    requested = path.resolve(outputDir);
  } else {
    ...
    requested = path.resolve(runRoot, outputDir);
...
function ensureReviewHandoffCard(inputArgs, outputDir) {
  ...
  if (!fs.existsSync(handoffPath)) {
    ...
    const handoffCopy = {
      ...
      es: {
        product: "Preparación del expediente de cliente",
...
function nextActionsWithReviewApplication(currentNextActions, appliedDecisions, blockers, language) {
  ...
  const copy = {
    ...
    es: {
      blockers: "Resolver las decisiones de revisión bloqueadas antes de considerar listos los artefactos finales.",
      regenerate: "Volver a generar las salidas nativas DOCX/XLSX/PDF antes de la entrega final.",
      ready: "Usar final_artifacts.json como galería revisada de artefactos para la entrega.",
      partial: "Completar las decisiones de revisión pendientes antes de la entrega final.",
    },
  }[language] || {
    blockers: "Resolve blocked review decisions before treating final artifacts as ready.",
    regenerate: "Regenerate native DOCX/XLSX/PDF outputs before final handoff.",
