...
description: "Use inside New Client to prepare an Italy, Geneva, Zurich, or UK client file: scan files, classify documents, identify missing or uncertain material, run formal checks, and produce structured outputs."
---
...
If the user does not provide `--out`, use a sibling `output/client-file-preparation`
...
4. When `newClientFilePreparation` MCP tools are available, use the OpenAI-style UI
   path: call `validate_client_file_preparation_review` with the complete payload, then
   call `render_client_file_preparation_review` once validation succeeds. The MCP server
