...
Evidence snapshot: 2026-08-10. This matrix separates code that is locally
...
| Statutory, OIC, disclosure, and filing instructions are effective-dated, behaviorally applied, and checksum-bound | `PROVED_LOCAL` | controlled statutory/OIC/Registry packs; OIC-pack differential questionnaire probe; period, campaign, source-register, early-adoption, identifier-only migration, and checksum-lineage tests | Accounting owner and rights sign-off remains external |
...
| Parse CSV/XLSX and reviewed readable/scanned PDF trial balances with source anchors | `PROVED_LOCAL` | `ingest_trial_balance`, `ingest_pdf_trial_balance`, `record_pdf_trial_balance_review`; CSV/workbook safety plus real text-PDF geometry, simulated OCR, low-confidence attention, all-page OCR closure, headerless continuations, page/table coverage binding, explicit empty-page dispositions, reconciled summary exclusions, mandatory declarations, atomic rejection, symlink, HTTP promotion, queued retry, and OCR package/model receipt-tampering tests | Direct accounting-package/API adapters and representative real-world PDF layout/OCR corpus evaluation are later scope |
...
| Calculate eligible forms through effective-dated rules and record user choice | `PROVED_LOCAL` | 2016–2023 and 2024–2026 form packs; `determine_forms`, `select_form`; explicit entry/continuation transition cases, non-negative metric gate, form-change consequences, first-year and exclusion tests; silent pack replacement rejection; explicit open-case migration/change report/full-revalidation tracking | Regulatory rule ownership/sign-off remains operational |
| Require every account to be mapped, split, or zero-balance excluded | `PROVED_LOCAL` | reviewed mapping decisions, collection-PATCH preservation probe, non-zero exclusion mutation and defensive-validation probes, exact split checks, coverage blocker, tenant/client mapping memory | Semantic mapping quality still needs representative model evaluation |
...
| Provide fixed-asset, inventory, receivable, payable, equity, provision, TFR, and tax schedules when triggered | `PROVED_LOCAL` | `scripts/schedule_engine.py`; equation, maturity, secured-payable, inventory valuation-evidence, sign, movement, statement, source-anchor, and template-ingestion tests | None for the normalized professional schedule contracts |
| Bind schedule evidence to form-specific PCI note tables | `PARTIAL` | `scripts/schedule_taxonomy_adapter.py`, checksum-locked adapter pack, exact per-cell dispositions, deterministic value derivation, primary-fact reconciliation, official inventory audit, and nine representative schedule golden workflows | Complete real-case professional bindings for every applicable table cell and dimension have not been exercised |
...
| Generate deterministic XBRL | `PROVED_LOCAL` | checksum-bound renderer, explicit sign multipliers, context/unit/decimals, text, dimension, tuple and nil tests; first-year sources and facts use one annuality with no comparative context; all ordinary, abbreviated and micro primary-presentation facts rendered; controlled schedule facts, including repeated tuple rows, are derived, reconciled and rendered | Complete real-case schedule and note-table filing content remains partial |
...
| Export XBRL, preview, mapping, issue, validation, and workpaper artifacts | `PROVED_LOCAL` | approved-snapshot export; staged atomic review/export publication and failed-job retry probes; exact catalogue/candidate binding; validator-mutation rejection; frozen approval audit trail; repeat-export byte identity; peer and manifest checksum tests | Production object storage is not implemented |
...
| Tenant and client data do not cross mapping, history, service, or path boundaries | `PROVED_LOCAL` | tenant authorization/memory/history tests; configured input-root, direct-symlink, and ancestor-symlink probes | Production database/object-store row policies are not implemented |
| Vera privacy surface is reviewed and current for this workstream | `PROVED_LOCAL` | `plugins/vera/privacy/workstreams/bilancio-xbrl-it.json` with refreshed source fingerprint; full privacy-surface validator passes on 2026-08-10 | None in the current repository register |
...
| All 24 minimum golden scenarios exist | `PROVED_LOCAL` | `evals/golden_cases.json`, `scripts/run_golden_cases.py`, golden registry/runner tests; all nine non-cash schedule families use a complete adapter disposition and emit representative official facts where applicable | The current instances prove balanced totals and representative scenario concepts, not complete statutory filing content |
| Controlled official-taxonomy run | `PROVED_LOCAL` | 24/24 pass on 2026-08-10; 20/20 public-lifecycle XBRL workflows pass; corrected first-year case has a current-only source and no comparative fact/context; four boundary passes; nine non-cash schedule cases complete the controlled taxonomy adapter; checksums in `references/taxonomy-spike.md` | External TEBENI/rendering comparison is open |
...
| Derived-output revision and version lineage | `PROVED_LOCAL` | Standard computation contexts on parser, eligibility, mapping-candidate, statement, schedule, disclosure, note, intelligence, preview, validation, and local-XBRL outputs include pack versions, all regulatory pack checksums, filing campaign, taxonomy checksum, and context contract tests | Production storage must preserve the same fields |
| Availability and recovery | `DEPLOYMENT_OPEN` | Atomic checksum-verified case writes, locks, job replay, staged artifact publication, failed-job retry probes, and reproducible approved exports exist | Production availability, backup restore and disaster-recovery evidence are absent |
