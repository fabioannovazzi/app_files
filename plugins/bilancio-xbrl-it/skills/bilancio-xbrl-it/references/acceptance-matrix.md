...
| Statutory, OIC, disclosure, and filing instructions are effective-dated and checksum-bound | `PROVED_LOCAL` | controlled statutory/OIC/Registry packs; period, campaign, source-register, unknown-label, early-adoption, migration, and checksum-lineage tests | Accounting owner and rights sign-off remains external |
...
| Calculate eligible forms through effective-dated rules and record user choice | `PROVED_LOCAL` | 2016–2023 and 2024–2026 form packs; `determine_forms`, `select_form`; explicit entry/continuation transition cases, form-change consequences, first-year and exclusion tests; silent pack replacement rejection; explicit open-case migration/change report/full-revalidation tracking | Regulatory rule ownership/sign-off remains operational |
...
| Export XBRL, preview, mapping, issue, validation, and workpaper artifacts | `PROVED_LOCAL` | approved-snapshot export; exact catalogue/candidate binding; validator-mutation rejection; frozen approval audit trail; repeat-export test proves byte-identical XBRL, preview, reports, workpaper, and manifest; peer checksums embedded in workpaper; final checksum manifest tests | Production object storage is not implemented |
