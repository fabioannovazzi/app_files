...
`retail_monthly.csv` fixture. The data is invented for Reporting Engine tests;
it is not customer data and makes no real-world business claim.

## Grain And Scope

Each row is one SKU-brand observation for one calendar month. The fixture has
four brands, one SKU per brand, two categories, two regions, and January-February
observations for 2025 and 2026. `Date` is the first day of the represented
calendar month, not a transaction date.

The reviewed comparison scope is January-February 2026 (`AC`) versus the same
calendar months in 2025 (`PY`). Any chart that uses `AC` or `PY` must also display
or carry those date boundaries. An all-available-data scope means all four
months in the fixture and must be requested explicitly.
...
The fixture supports monthly sales trajectory, current-versus-prior monthly
gaps, brand ranking for a bounded period, category sales mix over time,
...
decomposition between the reviewed AC and PY windows.
