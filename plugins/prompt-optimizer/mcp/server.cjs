...
const SERVER_NAME = "prompt-optimizer-widgets";
...
const WIDGET_URI = "ui://widget/prompt-optimizer-review.html";
...
const TOOL_NAMES = {
  validateReview: "validate_prompt_optimizer_review",
  renderReview: "render_prompt_optimizer_review",
  saveDecisions: "save_prompt_optimizer_decisions",
  applyDecisions: "apply_prompt_optimizer_decisions",
};
...
const RUNTIME_COPY = {
  en: {
    ...
    validationMessage:
      "Prompt Optimizer review is valid. Call render_prompt_optimizer_review with the returned short-lived reference.",
    saved: (count) => `Saved ${count} Prompt Optimizer decisions.`,
    saveNotWritten:
      "Validated decisions. No run_intake.output_dir was provided, so nothing was written.",
    applied: (count) => `Applied ${count} Prompt Optimizer decisions.`,
    applyNotWritten:
    ...
    instructions:
      "Use validate_prompt_optimizer_review with the hash-bound local review reference before render_prompt_optimizer_review, then reuse the returned short-lived token. Prefer the MCP widget for final Prompt Optimizer package review; use save_prompt_optimizer_decisions to persist review actions and apply_prompt_optimizer_decisions to write applied_decisions.json plus final_artifacts.json status; use native Plan-mode choices or chat for simple pre-draft intake decisions.",
  },
  es: {
    ...
    validationMessage:
      "La revisión del optimizador de prompts es válida. Ejecute render_prompt_optimizer_review con la referencia temporal devuelta.",
    saved: (count) =>
      `Se ${count === 1 ? "ha" : "han"} guardado ${count} ${count === 1 ? "decisión" : "decisiones"} del optimizador de prompts.`,
    saveNotWritten:
      "Las decisiones son válidas. No se ha proporcionado run_intake.output_dir, por lo que no se ha escrito ningún archivo.",
    applied: (count) =>
      `Se ${count === 1 ? "ha" : "han"} aplicado ${count} ${count === 1 ? "decisión" : "decisiones"} del optimizador de prompts.`,
    applyNotWritten:
    ...
    instructions:
      "Ejecute validate_prompt_optimizer_review con la referencia local vinculada por hash antes de render_prompt_optimizer_review y reutilice después el token temporal devuelto. Utilice preferentemente el widget MCP para la revisión final del paquete; guarde las decisiones con save_prompt_optimizer_decisions y aplíquelas con apply_prompt_optimizer_decisions para generar applied_decisions.json y actualizar el estado de final_artifacts.json. Para decisiones sencillas previas al borrador, utilice las opciones nativas del modo Plan o el chat.",
  },
};
...
function toolUiMeta(resourceUri, toolName = null) {
  ...
  if (toolName === TOOL_NAMES.renderReview) {
    meta["openai/toolInvocation/invoking"] = "Rendering Prompt Optimizer review";
    meta["openai/toolInvocation/invoked"] = "Rendered Prompt Optimizer review";
  }
  return meta;
}
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Prompt Optimizer review surface for validation checks, optimized prompt artifacts, source-domain sidecars, and package files.",
  ...
  return [
    {
      ...
      title: "Validate Prompt Optimizer review payload",
      description:
        "Validate a hash-bound local Prompt Optimizer review reference (or a legacy inline payload) before rendering. Call this first, then render_prompt_optimizer_review with the returned short-lived token.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Prompt Optimizer review",
      description:
        "Render a validated Prompt Optimizer review through its short-lived token as an MCP HTML widget for validation checks, optimized prompt artifacts, source-domain sidecars, and package files.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Prompt Optimizer review decisions",
      description:
        "Validate Prompt Optimizer review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Prompt Optimizer review decisions",
      description:
        "Validate Prompt Optimizer review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: true,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "prompt_optimizer_review_widget",
      title: "Prompt Optimizer review widget",
      description:
        "Renders Prompt Optimizer review-session payloads with searchable validation checks and generated artifacts.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
  throw new Error(`unknown Prompt Optimizer widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "prompt-optimizer-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "prompt-optimizer") {
    throw new Error('review_payload.plugin must be "prompt-optimizer"');
  }
  ...
  const payload = {
    widget_type: "prompt_optimizer_review",
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`Prompt Optimizer widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function storedReviewContext(inputArgs, expectedReview = null) {
  ...
  if (
    finalArtifacts.plugin !== "prompt-optimizer" ||
    finalArtifacts.workflow !== "prompt-optimizer" ||
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "prompt_optimizer_decisions",
...
function resolveRunOutputDir(inputArgs) {
  ...
  if (!contextRef) {
    throw new Error(
      "Prompt Optimizer persistence requires the current client_engagement context",
    );
  }
  if (
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "prompt_optimizer"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function ensureReviewHandoffCard(inputArgs, outputDir) {
  ...
  if (!fs.existsSync(handoffPath)) {
    ...
    const text = [
      spanish ? `# ${displayName} · ${copy.handoffTitle}` : `# ${displayName} Review Handoff`,
      ...(spanish ? ["<!-- review-contract: Review Handoff -->"] : []),
      "",
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "prompt_optimizer_application",
...
function preflightClientRun(outputDir, expectedRunId) {
  ...
  if (completed.status !== 0) {
    throw new Error(
      completed.stderr ||
        completed.stdout ||
        "Prompt Optimizer customer-run preflight failed.",
    );
  }
  const output = completed.stdout.trim().split(/\r?\n/).filter(Boolean).pop();
  if (!output) throw new Error("Prompt Optimizer customer-run preflight failed.");
  const parsed = JSON.parse(output);
  if (
    !isPlainObject(parsed) ||
    parsed.ok !== true ||
    parsed.schema_version !== "vera.client_workflow_context.v2" ||
    parsed.workflow_id !== "prompt-optimizer" ||
    typeof parsed.client_run_id !== "string" ||
    !parsed.client_run_id.trim() ||
    parsed.client_run_id !== expectedRunId
  ) {
    throw new Error("Prompt Optimizer customer-run preflight returned an invalid result.");
  }
  return parsed;
}
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    return {
      ok: true,
      validation_type: "prompt_optimizer_review",
  ...
  throw new Error(`unknown Prompt Optimizer widget tool: ${name}`);
}
  ...
  try {
    if (method === "initialize") {
      return rpcResponse(messageId, {
        ...
        instructions: runtimeCopy(params).instructions,
      });
    }
