...
const SERVER_NAME = "distribution-widgets";
...
const WIDGET_URI = "ui://widget/distribution-review.html";
const WIDGET_MIME_TYPE = "text/html;profile=mcp-app";
const MAX_ITEMS = 2500;
const MAX_PAYLOAD_BYTES = 2_000_000;
const TOOL_NAMES = {
  validateReview: "validate_distribution_review",
  renderReview: "render_distribution_review",
  saveDecisions: "save_distribution_decisions",
  applyDecisions: "apply_distribution_decisions",
};
...
function widgetResourceMeta(uri) {
  return {
    ...
    "openai/widgetDescription":
      "Interactive Distribution Analysis review surface for summary statistics, generated charts, source contexts, follow-up requests, and final artifacts.",
  ...
  return [
    {
      ...
      title: "Validate Distribution review payload",
      description:
        "Validate the Distribution Analysis review-session payload before rendering. Call this first, then render_distribution_review.",
      inputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    },
    {
      ...
      title: "Render Distribution review",
      description:
        "Render a Distribution Analysis review-session payload as an MCP HTML widget for summary statistics, charts, source contexts, follow-ups, and artifacts.",
      inputSchema,
    ...
    },
    {
      ...
      title: "Save Distribution review decisions",
      description:
        "Validate Distribution review decisions and persist them to ui_decisions.json when run_intake.output_dir is available.",
    ...
    },
    {
      ...
      title: "Apply Distribution review decisions",
      description:
        "Validate Distribution review decisions, write applied_decisions.json, and update final_artifacts.json status when run_intake.output_dir is available.",
      inputSchema: decisionInputSchema,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
function resources() {
  return [
    {
      uri: WIDGET_URI,
      name: "distribution_review_widget",
      title: "Distribution review widget",
      description:
        "Renders Distribution Analysis review-session payloads with searchable summary rows, chart artifacts, contexts, and follow-up requests.",
...
function resourceText(uri) {
  if (uri !== WIDGET_URI) {
    throw new Error(`unknown Distribution widget resource: ${uri}`);
  }
  return fs.readFileSync(
    path.join(PLUGIN_ROOT, "assets", "distribution-review-widget.html"),
    "utf8",
  );
}
...
function validateReviewPayload(inputArgs) {
  ...
  if (reviewPayload.plugin !== "distribution-analysis") {
    throw new Error('review_payload.plugin must be "distribution-analysis"');
  }
  ...
  if (payloadBytes(payload) > MAX_PAYLOAD_BYTES) {
    throw new Error(`Distribution widget payload exceeds ${MAX_PAYLOAD_BYTES} bytes`);
  }
  return payload;
}
...
function saveDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "distribution_decisions",
...
function appendReviewApplicationExecutionTrace(
  ...
  trace.push({
    step_id: `${shortString(appliedDecisions?.workflow) || "distribution"}_review_apply_${stepIdSuffix || Date.now()}`,
...
function applyDecisionPayload(inputArgs) {
  ...
  return {
    ok: true,
    validation_type: "distribution_application",
...
function callTool(name, args = {}) {
  if (name === TOOL_NAMES.validateReview) {
    ...
    return {
      ok: true,
      validation_type: "distribution_review",
  ...
  throw new Error(`unknown Distribution widget tool: ${name}`);
}
