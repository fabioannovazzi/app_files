...
from modules.pdp.purina_catalog import PURINA_CATEGORY_KEY, PURINA_RETAILER
from modules.pdp.purina_filter_discovery import build_purina_filter_records
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize Purina PDP site filters as retailer_filter attribute "
    ...
    parser.add_argument(
        ...
        help="Category keys to materialize (default: wet_cat_food).",
    )
    parser.add_argument(
        "--filter-families",
        nargs="*",
    ...
    *,
    ...
    try:
        with connect_pdp_database(pdp_store_path) as conn:
            raw_rows = conn.execute(query, (PURINA_RETAILER,)).fetchall()
    except Exception as exc:
        LOGGER.warning("Unable to load Purina parent rows: %s", exc)
        return rows

    for parent_id, pdp_url, raw_extras in raw_rows:
        extras = _parse_extras(raw_extras)
        category_key = (
            str(extras.get("category_key") or PURINA_CATEGORY_KEY).strip().lower()
        )
...
def _write_evidence(
    *,
    ...
    run_dir = (
        evidence_root / PURINA_RETAILER / crawl_ts.replace(":", "").replace("+", "Z")
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    surfaces, observations = build_purina_filter_records(
        parents,
    ...
    LOGGER.info(
        (
            "Materialized Purina filters: parents=%d surfaces=%d "
            "observations=%d attribute_rows=%d evidence=%s"
        ),
        len(parents),
        len(surfaces),
        len(observations),
