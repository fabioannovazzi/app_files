...
from modules.pdp.tikicat_catalog import TIKICAT_CATEGORY_KEY, TIKICAT_RETAILER
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize Tiki Cat PDP site filters as retailer_filter attribute "
    ...
    *,
    ...
    try:
        with connect_pdp_database(pdp_store_path) as conn:
            raw_rows = conn.execute(query, (TIKICAT_RETAILER,)).fetchall()
    except Exception as exc:
        LOGGER.warning("Unable to load Tiki Cat parent rows: %s", exc)
        return rows

    for parent_id, pdp_url, raw_extras in raw_rows:
        extras = _parse_extras(raw_extras)
        category_key = (
            str(extras.get("category_key") or TIKICAT_CATEGORY_KEY).strip().lower()
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    surfaces, observations = build_tikicat_filter_records(
        parents,
    ...
    LOGGER.info(
        (
            "Materialized Tiki Cat filters: parents=%d surfaces=%d "
