...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run only the image/VLM PDP attribute enrichment step."
    )
    parser.add_argument(
        "--retailer",
        action="append",
...
def main() -> None:
    """Run the image-only attribute mapper without the web-search pass."""
    args = _parse_args()
    logging.basicConfig(
        level=logging.INFO,
    ...
    run_attribute_mapping_vlm(retailers=args.retailer)
