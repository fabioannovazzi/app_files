...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize Guest in Residence PDP site filters as retailer_filter "
            "attribute observations for parsed parent products."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Category keys to materialize (default: cashmere_sweaters).",
    )
    parser.add_argument(
        "--filter-families",
        nargs="*",
    ...
    *,
    ...
    try:
        with connect_pdp_database(pdp_store_path) as conn:
            raw_rows = conn.execute(query, (GUESTINRESIDENCE_RETAILER,)).fetchall()
    except Exception as exc:
        LOGGER.warning("Unable to load Guest in Residence parent rows: %s", exc)
        return rows

    for parent_id, pdp_url, raw_extras in raw_rows:
        extras = _parse_extras(raw_extras)
        category_key = (
            str(extras.get("category_key") or GUESTINRESIDENCE_CATEGORY_KEY)
            .strip()
            .lower()
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    LOGGER.info(
        (
            "Materialized Guest in Residence filters: parents=%d surfaces=%d "
