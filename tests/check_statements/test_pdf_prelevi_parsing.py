...
def test_parse_pdf_prepare_extracts_prelevi(
    pdf_name: str, expected: tuple[date, float]
) -> None:
    ...
    if not content.is_file():
        pytest.skip(f"Optional local PDF fixture is unavailable: {pdf_name}")
    try:
    ...
    for row in rows:
        if (
            ...
            and pytest.approx(row.get("amount"), abs=0.01) == target_amount
        ):
            match = row
            break
