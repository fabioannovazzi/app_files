...
def _load_core() -> Any:
    return _load_module(
        "client_onboarding_core_contract_test",
        PLUGIN_ROOT / "scripts" / "onboarding_core.py",
    )
...
def _node_binary() -> str:
    ...
    if node is None:
        pytest.skip("Node.js is required for the client-onboarding MCP tests.")
    return node
...
def _generate_package(tmp_path: Path) -> Path:
    output_dir = tmp_path / "private-client-onboarding"
    ...
    assert json.loads(initialized.stdout)["status"] == "intake_initialized"
...
def test_vera_declares_client_onboarding_component_skill_and_mcp_route() -> None:
    ...
    assert "client-onboarding" in components["plugins"]
    assert "client-onboarding" in routed_modules
    wrapper = VERA_ROOT / "skills" / "client-onboarding" / "SKILL.md"
    wrapper_text = wrapper.read_text(encoding="utf-8")
    assert "modules/client-onboarding" in wrapper_text
    assert "skills/client-onboarding/SKILL.md" in wrapper_text


def test_vera_delegates_client_onboarding_dependency_check() -> None:
...
def test_vera_zip_expected_entries_embed_client_onboarding_component() -> None:
    builder = _load_module("build_vera_client_onboarding", BUILD_SCRIPT)
    bundle = next(bundle for bundle in builder.load_bundles() if bundle.name == "vera")

    entries = builder.expected_zip_entries(bundle)

    prefix = "vera-codex-plugin/plugins/vera/modules/client-onboarding/"
    for relative_path in (
        "skills/client-onboarding/SKILL.md",
        "scripts/initialize_case.py",
        "scripts/package_onboarding.py",
        "scripts/onboarding_core.py",
        "schemas/onboarding_intake.schema.json",
...
def test_packaged_vera_runs_client_onboarding_through_dispatcher(
    tmp_path: Path,
) -> None:
    builder = _load_module("build_packaged_vera_client_onboarding", BUILD_SCRIPT)
    bundle = next(bundle for bundle in builder.load_bundles() if bundle.name == "vera")
...
def test_client_onboarding_generated_workflow_passes_both_contracts(
    tmp_path: Path,
) -> None:
    ...
    assert run_intake["plugin"] == "client-onboarding"
...
def test_client_onboarding_render_exposes_only_safe_output_basenames(
    tmp_path: Path,
) -> None:
    output_dir = _generate_package(tmp_path)
    run_intake = json.loads(
...
def test_client_onboarding_mcp_enforces_semantic_and_document_request_notes(
    tmp_path: Path,
) -> None:
    output_dir = _generate_package(tmp_path)
    run_intake = json.loads(
...
def test_client_onboarding_mcp_rejects_open_ended_review_status() -> None:
...
def test_client_onboarding_mcp_requires_current_contract_version() -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_mcp_rejects_unknown_item_data() -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_mcp_rejects_non_decision_item_contracts(
    item_type: str,
) -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_mcp_requires_all_source_artifact_bindings() -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_mcp_requires_complete_basis_hashes() -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_mcp_accepts_declared_compliance_item_contracts(
    item_type: str,
    data: dict[str, Any],
) -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_stateful_tools_are_not_declared_idempotent() -> None:
    response = _call_mcp(
        [
            {
                "jsonrpc": "2.0",
...
def test_client_onboarding_mcp_rejects_output_inside_source_package() -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_save_apply_preserves_integrity_and_history(
    tmp_path: Path,
) -> None:
    output_dir = _generate_package(tmp_path)
    core = _load_core()
    ...
    saved = _call_mcp([_tool_call(1, "save_client_onboarding_decisions", common)])[1][
        "result"
    ]["structuredContent"]
...
def test_client_onboarding_save_rejects_stale_decision_revision(
    tmp_path: Path,
) -> None:
    output_dir = _generate_package(tmp_path)
    run_intake = json.loads(
    ...
    first = _call_mcp([_tool_call(1, "save_client_onboarding_decisions", arguments)])[
        1
    ]["result"]["structuredContent"]
    ui_after_first = (output_dir / "ui_decisions.json").read_bytes()
    final_after_first = (output_dir / "final_artifacts.json").read_bytes()
    stale = _call_mcp([_tool_call(1, "save_client_onboarding_decisions", arguments)])[
        1
    ]["result"]
...
def test_client_onboarding_save_rejects_tampered_local_artifact_before_writing(
    tmp_path: Path,
) -> None:
    output_dir = _generate_package(tmp_path)
    run_intake = json.loads(
...
def test_client_onboarding_save_rejects_incomplete_export_gate_before_writing(
    tmp_path: Path,
) -> None:
    output_dir = _generate_package(tmp_path)
    run_intake = json.loads(
...
def test_client_onboarding_preview_cannot_claim_professional_export_readiness() -> None:
    review_payload = _strict_review_payload()
    final_artifacts = {
        "schema_version": "1.1",
        "contract_version": "1.1",
...
def test_client_onboarding_edit_requires_revision_before_export() -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_marketing_blocker_does_not_block_relationship() -> None:
    review_payload = _strict_review_payload()
...
def test_client_onboarding_widget_state_excludes_free_text_decisions() -> None:
    adapter = json.loads(
    ...
    widget = (
        PLUGIN_ROOT / "assets" / "client-onboarding-review-widget.html"
...
def test_client_onboarding_public_page_explains_the_operational_journey() -> None:
    page = (ROOT / "static" / "shared" / "client-onboarding" / "index.html").read_text(
        encoding="utf-8"
    )

    for snippet in (
        "Client Onboarding",
        "Dal cliente al fascicolo operativo.",
        "Cosa fornisci",
        "Cosa prepara Vera",
        "Cosa ricevi",
        "L’onboarding continua ciò che l’istruttoria ha iniziato.",
        "RI 30% + RS 70%",
        "Mandato",
        "Privacy",
        "Informativa AI",
        ...
        'href="../client-intake/index.html?lang=it"',
        'id="prompt-example"',
    ):
