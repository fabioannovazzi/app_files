#!/usr/bin/env python3
"""
run_codex_print_to_logging_refactor.py

# run the refactor on every *.py file under the paths listed in modules_to_scan.txt
# and commit the changes in one commit
./run_codex_print_to_logging_refactor.py \
  --paths-from modules_to_scan.txt \
  --all \
  --git-commit \
  --git-message "codex: replace print with logging/ui"

Enforce guideline:
| Use the `logging` module or UI adapter display functions instead of `print` for diagnostics. |

- Detects print(...) calls outside of tests and replaces with logging.* or UI adapter calls when appropriate.
- Inserts `import logging` when needed; keeps behavior minimal & focused.
- Single commit at end if --git-commit is passed.
"""
...
def is_candidate_file(p: Path) -> bool:
    ...
    return bool(PRINT_RE.search(code))
...
WHAT TO CHANGE
1) Replace print(...) calls:
   - Prefer `logging.info(...)` or, inside exception handlers, `logging.exception(...)`.
   - If a module clearly uses UI adapter hooks, replace with that adapter's display call instead of logging.

2) Insert `import logging` at top-level if not present (after stdlib imports). Do not duplicate imports.

3) Preserve formatting and variables; avoid changing program flow. Do NOT alter `print` used for strict file output (e.g., CLI stdout for a machine consumer) unless obviously diagnostic.

4) Idempotency: do not re‑convert lines that are already using logging or UI adapter helpers.
""".strip()
...
def main():
    ap=argparse.ArgumentParser(description="Convert print diagnostics to logging or UI adapters")
    ...
    ap.add_argument("--git-message", default="codex: replace print with logging/ui")
    args=ap.parse_args()
    ...
    if not candidates:
        raise SystemExit("No Python files matched patterns." if args.all else "No print(...) diagnostics found.")
    ...
    if args.git_commit:
        ...
        if proc.returncode==0:
            ...
            print(f"[codex-print] Committed as {short}")
        else:
            print("[codex-print] No commit performed. Git said:"); print(proc.stdout or proc.stderr)
